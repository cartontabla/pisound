...
/* Real-time model */
static RT_MODEL_wah_T wah_M_;
RT_MODEL_wah_T *const wah_M = &wah_M_;

/* Forward declaration for local functions */
static real_T wah_wt_read_lin(const real_T tbl[4096], real_T L, real_T ph);
static void wah_filter(real_T b[3], real_T a[3], const real_T x[128], const
...
void wah_step(void)
{
  real_T rtb_MATLABSystem1_o1_0[128];
  ...
  {
    SimStruct *rts = wah_M->childSfunctions[0];
    sfcnOutputs(rts,0);
  }
  ...
  ws = wah_P.WaveSel_rt;
  if (wah_P.WaveSel_rt < 1) {
    ws = 1U;
  } else if (wah_P.WaveSel_rt > 10) {
    ws = 10U;
  }

  L = wah_DW.obj.TablesSize[ws - 1];
  if (L < 2.0) {
    L = 2.0;
  }

  inc = wah_P.Rate_rt / 48000.0;
  ph = wah_DW.obj.Phase;
  if (rtIsNaN(wah_P.iPhase_rt) || rtIsInf(wah_P.iPhase_rt)) {
    q = (rtNaN);
  } else {
    q = fabs(wah_P.iPhase_rt / 6.2831853071795862);
    if (fabs(q - floor(q + 0.5)) > 2.2204460492503131E-16 * q) {
      q = fmod(wah_P.iPhase_rt, 6.2831853071795862);
    } else {
      q = 0.0;
    }
  ...
  /*  cycles */
  m = (wah_P.Constant12_Value != 0);
  for (t = 0; t < 128; t++) {
    vL = wah_wt_read_lin(&wah_DW.obj.WaveTables[(ws - 1) << 12], L, ph + q);
    if (m) {
      /*  unipolar 0..1 */
      vL = (vL + 1.0) * 0.5;
    } else {
    ...
    rtb_MATLABSystem1_o1_0[t] = wah_P.Constant8_Value * vL;
    ph += inc;
    if (ph >= 1.0) {
      ph--;
    }
  ...
  L = wah_P.Fmin_rt;
  inc = wah_P.Fmax_rt;
  ph = rtb_MATLABSystem1_o1_0[0];
  for (t = 0; t < 127; t++) {
    ph += rtb_MATLABSystem1_o1_0[t + 1];
  }

  if (wah_P.Fmax_rt < wah_P.Fmin_rt) {
    L = wah_P.Fmax_rt;
    inc = wah_P.Fmin_rt;
  }
  ...
  L = exp(log(inc / L) / 2.0 * (fmax(-1.0, fmin(1.0, ph / 128.0)) * fmax(0.0,
            fmin(1.0, wah_P.Depth_rt)))) * sqrt(L * inc);
  ...
  if ((wah_P.Type_rt != wah_DW.obj_i.lastType) || (L != wah_DW.obj_i.lastFreq) ||
      (wah_P.Q_rt != wah_DW.obj_i.lastQ) || (wah_P.FilterGain_rt !=
       wah_DW.obj_i.lastGain)) {
    q = L;
    inc = wah_P.Q_rt;
    ph = wah_P.FilterGain_rt;
    t = wah_P.Type_rt;
    if (rtIsInf(L) || rtIsNaN(L)) {
      q = 1000.0;
    }

    if (rtIsInf(wah_P.Q_rt) || rtIsNaN(wah_P.Q_rt)) {
      inc = 0.707;
    }

    if (rtIsInf(wah_P.FilterGain_rt) || rtIsNaN(wah_P.FilterGain_rt)) {
      ph = 0.0;
    }
    ...
    q = q / 48000.0 * 6.2831853071795862;
    if (wah_P.Type_rt < 1) {
      t = 1;
    } else if (wah_P.Type_rt > 7) {
      t = 7;
    }

    wah_DW.obj_i.b[0] = 1.0;
    wah_DW.obj_i.a[0] = 1.0;
    wah_DW.obj_i.b[1] = 0.0;
    wah_DW.obj_i.a[1] = 0.0;
    wah_DW.obj_i.b[2] = 0.0;
    wah_DW.obj_i.a[2] = 0.0;
    switch ((uint8_T)t) {
     case 1U:
      /*  lowpass */
      ...
      p = cos(q);
      vL = 1.4142135623730951 * sin(q);

      /* Normalización de coeficientes */
      /* fvtool(b,a); */
      ph = (1.0 - p) / (vL + 2.0);
      wah_DW.obj_i.b[0] = ph;
      wah_DW.obj_i.b[1] = (1.0 - p) * 2.0 / (vL + 2.0);
      wah_DW.obj_i.b[2] = ph;
      wah_DW.obj_i.a[0] = (vL + 2.0) / (vL + 2.0);
      wah_DW.obj_i.a[1] = -4.0 * p / (vL + 2.0);
      wah_DW.obj_i.a[2] = (2.0 - vL) / (vL + 2.0);
      break;
     ...
     case 2U:
      ...
      ph = (p + 1.0) / (vL + 2.0);
      wah_DW.obj_i.b[0] = ph;
      wah_DW.obj_i.b[1] = (p + 1.0) * -2.0 / (vL + 2.0);
      wah_DW.obj_i.b[2] = ph;
      wah_DW.obj_i.a[0] = (vL + 2.0) / (vL + 2.0);
      wah_DW.obj_i.a[1] = -4.0 * p / (vL + 2.0);
      wah_DW.obj_i.a[2] = (2.0 - vL) / (vL + 2.0);
     ...
     case 3U:
      ...
      /* Normalización de coeficientes */
      /* fvtool(b,a); */
      wah_DW.obj_i.b[0] = vL / (vL + 2.0);
      wah_DW.obj_i.b[1] = 0.0 / (vL + 2.0);
      wah_DW.obj_i.b[2] = -vL / (vL + 2.0);
      wah_DW.obj_i.a[0] = (vL + 2.0) / (vL + 2.0);
      wah_DW.obj_i.a[1] = -4.0 * cos(q) / (vL + 2.0);
      wah_DW.obj_i.a[2] = (2.0 - vL) / (vL + 2.0);
      break;
     ...
     case 4U:
      ...
      ph = 2.0 / (vL + 2.0);
      wah_DW.obj_i.b[0] = ph;
      tmp_0 = -4.0 * cos(q) / (vL + 2.0);
      wah_DW.obj_i.b[1] = tmp_0;
      wah_DW.obj_i.b[2] = ph;
      wah_DW.obj_i.a[0] = (vL + 2.0) / (vL + 2.0);
      wah_DW.obj_i.a[1] = tmp_0;
      wah_DW.obj_i.a[2] = (2.0 - vL) / (vL + 2.0);
      break;
     ...
     case 5U:
      ...
      /* fvtool(b,a); */
      ph = (1.0 - A) * p + (A + 1.0);
      tmp_0 = 1.4142135623730951 * sqrtA * q;
      wah_DW.obj_i.b[0] = (ph + tmp_0) / a0;
      wah_DW.obj_i.b[1] = ((A + 1.0) * p + (1.0 - A)) * -2.0 / a0;
      wah_DW.obj_i.b[2] = (ph - tmp_0) / a0;
      wah_DW.obj_i.a[0] = a0 / a0;
      wah_DW.obj_i.a[1] = ((vL + 1.0) * p + (1.0 - vL)) * -2.0 / a0;
      wah_DW.obj_i.a[2] = (a0_tmp - a0_tmp_0) / a0;
      break;
     ...
     case 6U:
      ...
      /* fvtool(b,a); */
      ph = 1.0 / A;
      tmp_0 = (1.0 - ph) * p + (ph + 1.0);
      q = 1.4142135623730951 * q / sqrtA;
      wah_DW.obj_i.b[0] = (tmp_0 + q) * a0_tmp_0 / a0;
      wah_DW.obj_i.b[1] = ((ph + 1.0) * p + (1.0 - ph)) * -2.0 * a0_tmp_0 / a0;
      wah_DW.obj_i.b[2] = (tmp_0 - q) * a0_tmp_0 / a0;
      wah_DW.obj_i.a[0] = a0 / a0;
      wah_DW.obj_i.a[1] = ((A + 1.0) * p + (1.0 - A)) * -2.0 / a0;
      wah_DW.obj_i.a[2] = (vL - a0_tmp) / a0;
      break;
     ...
     case 7U:
      ...
      /* fvtool(b,a); */
      ph *= A;
      wah_DW.obj_i.b[0] = (ph + 1.0) / (vL + 1.0);
      tmp_0 = -2.0 * cos(q) / (vL + 1.0);
      wah_DW.obj_i.b[1] = tmp_0;
      wah_DW.obj_i.b[2] = (1.0 - ph) / (vL + 1.0);
      wah_DW.obj_i.a[0] = (vL + 1.0) / (vL + 1.0);
      wah_DW.obj_i.a[1] = tmp_0;
      wah_DW.obj_i.a[2] = (1.0 - vL) / (vL + 1.0);
      break;
    }

    wah_DW.obj_i.lastType = wah_P.Type_rt;
    wah_DW.obj_i.lastFreq = L;
    wah_DW.obj_i.lastQ = inc;
    wah_DW.obj_i.lastGain = wah_P.FilterGain_rt;
  }
  ...
  /*  bp es logical */
  for (t = 0; t < 128; t++) {
    tmp[t] = wah_P.Gain_Gain * (real_T)wah_B.PiSoundInput_o1[t];
  }
  ...
  for (t = 0; t < 2; t++) {
    tmp_1[t] = wah_DW.obj_i.zL[t];
  }

  for (t = 0; t < 3; t++) {
    tmp_2[t] = wah_DW.obj_i.b[t];
  }

  for (t = 0; t < 3; t++) {
    tmp_3[t] = wah_DW.obj_i.a[t];
  }

  wah_filter(tmp_2, tmp_3, tmp, tmp_1, rtb_MATLABSystem1_o1_0, wah_DW.obj_i.zL);

  /* Gain: '<Root>/Gain1' incorporates:
   *  DataTypeConversion: '<Root>/Data Type Conversion1'
   */
  for (t = 0; t < 128; t++) {
    tmp[t] = wah_P.Gain1_Gain * (real_T)wah_B.PiSoundInput_o2[t];
  }
  ...
  wah_filter(tmp_2, tmp_3, tmp, tmp_1, rtb_MATLABSystem_o2_0, wah_DW.obj_i.zR);
  ...
  /*  g es Nx1 */
  if (wah_P.Constant6_Value != 0) {
    L = rt_powd_snf(10.0, wah_P.Gain_rt / 20.0);
  ...
  for (t = 0; t < 128; t++) {
     ...
     * */
    ph = floor(rtb_MATLABSystem1_o1_0[t] * L * wah_P.Gain2_Gain);
    if (rtIsNaN(ph) || rtIsInf(ph)) {
      ph = 0.0;
    } else {
      ph = fmod(ph, 4.294967296E+9);
    }

    /* DataTypeConversion: '<Root>/Data Type Conversion2' */
    wah_B.DataTypeConversion2[t] = ph < 0.0 ? -(int32_T)(uint32_T)-ph : (int32_T)
      (uint32_T)ph;
     ...
     *  MATLABSystem: '<Root>/MATLAB System2'
     * */
    ph = floor(rtb_MATLABSystem_o2_0[t] * L * wah_P.Gain3_Gain);
    if (rtIsNaN(ph) || rtIsInf(ph)) {
      ph = 0.0;
    } else {
      ph = fmod(ph, 4.294967296E+9);
    ...
    wah_B.DataTypeConversion3[t] = ph < 0.0 ? -(int32_T)(uint32_T)-ph : (int32_T)
      (uint32_T)ph;
  }
  ...
  {
    SimStruct *rts = wah_M->childSfunctions[1];
    sfcnOutputs(rts,0);
  }
   ...
   */
  wah_M->Timing.t[0] =
    ((time_T)(++wah_M->Timing.clockTick0)) * wah_M->Timing.stepSize0;
}

/* Model initialize function */
void wah_initialize(void)
{
  /* Registration code */
  rtsiSetSolverName(&wah_M->solverInfo,"FixedStepDiscrete");
  wah_M->solverInfoPtr = (&wah_M->solverInfo);

  /* Initialize timing info */
  {
    int_T *mdlTsMap = wah_M->Timing.sampleTimeTaskIDArray;
    mdlTsMap[0] = 0;
    wah_M->Timing.sampleTimeTaskIDPtr = (&mdlTsMap[0]);
    wah_M->Timing.sampleTimes = (&wah_M->Timing.sampleTimesArray[0]);
    wah_M->Timing.offsetTimes = (&wah_M->Timing.offsetTimesArray[0]);

    /* task periods */
    wah_M->Timing.sampleTimes[0] = (0.0026666666666666666);

    /* task offsets */
    wah_M->Timing.offsetTimes[0] = (0.0);
  }

  rtmSetTPtr(wah_M, &wah_M->Timing.tArray[0]);

  {
    int_T *mdlSampleHits = wah_M->Timing.sampleHitArray;
    mdlSampleHits[0] = 1;
    wah_M->Timing.sampleHits = (&mdlSampleHits[0]);
  }

  rtmSetTFinal(wah_M, -1);
  wah_M->Timing.stepSize0 = 0.0026666666666666666;
  wah_M->solverInfoPtr = (&wah_M->solverInfo);
  wah_M->Timing.stepSize = (0.0026666666666666666);
  rtsiSetFixedStepSize(&wah_M->solverInfo, 0.0026666666666666666);
  rtsiSetSolverMode(&wah_M->solverInfo, SOLVER_MODE_SINGLETASKING);

  /* child S-Function registration */
  {
    RTWSfcnInfo *sfcnInfo = &wah_M->NonInlinedSFcns.sfcnInfo;
    wah_M->sfcnInfo = (sfcnInfo);
    rtssSetErrorStatusPtr(sfcnInfo, (&rtmGetErrorStatus(wah_M)));
    wah_M->Sizes.numSampTimes = (1);
    rtssSetNumRootSampTimesPtr(sfcnInfo, &wah_M->Sizes.numSampTimes);
    wah_M->NonInlinedSFcns.taskTimePtrs[0] = (&rtmGetTPtr(wah_M)[0]);
    rtssSetTPtrPtr(sfcnInfo,wah_M->NonInlinedSFcns.taskTimePtrs);
    rtssSetTStartPtr(sfcnInfo, &rtmGetTStart(wah_M));
    rtssSetTFinalPtr(sfcnInfo, &rtmGetTFinal(wah_M));
    rtssSetTimeOfLastOutputPtr(sfcnInfo, &rtmGetTimeOfLastOutput(wah_M));
    rtssSetStepSizePtr(sfcnInfo, &wah_M->Timing.stepSize);
    rtssSetStopRequestedPtr(sfcnInfo, &rtmGetStopRequested(wah_M));
    rtssSetDerivCacheNeedsResetPtr(sfcnInfo, &wah_M->derivCacheNeedsReset);
    rtssSetZCCacheNeedsResetPtr(sfcnInfo, &wah_M->zCCacheNeedsReset);
    rtssSetContTimeOutputInconsistentWithStateAtMajorStepPtr(sfcnInfo,
      &wah_M->CTOutputIncnstWithState);
    rtssSetSampleHitsPtr(sfcnInfo, &wah_M->Timing.sampleHits);
    rtssSetPerTaskSampleHitsPtr(sfcnInfo, &wah_M->Timing.perTaskSampleHits);
    rtssSetSimModePtr(sfcnInfo, &wah_M->simMode);
    rtssSetSolverInfoPtr(sfcnInfo, &wah_M->solverInfoPtr);
  }

  wah_M->Sizes.numSFcns = (2);

  /* register each child */
  {
    (void) memset((void *)&wah_M->NonInlinedSFcns.childSFunctions[0], 0,
                  2*sizeof(SimStruct));
    wah_M->childSfunctions = (&wah_M->NonInlinedSFcns.childSFunctionPtrs[0]);
    wah_M->childSfunctions[0] = (&wah_M->NonInlinedSFcns.childSFunctions[0]);
    wah_M->childSfunctions[1] = (&wah_M->NonInlinedSFcns.childSFunctions[1]);

    /* Level2 S-Function Block: wah/<Root>/PiSound Input (pisound_in) */
    {
      SimStruct *rts = wah_M->childSfunctions[0];

      /* timing info */
      time_T *sfcnPeriod = wah_M->NonInlinedSFcns.Sfcn0.sfcnPeriod;
      time_T *sfcnOffset = wah_M->NonInlinedSFcns.Sfcn0.sfcnOffset;
      int_T *sfcnTsMap = wah_M->NonInlinedSFcns.Sfcn0.sfcnTsMap;
      ...
      {
        ssSetBlkInfo2Ptr(rts, &wah_M->NonInlinedSFcns.blkInfo2[0]);
      }

      _ssSetBlkInfo2PortInfo2Ptr(rts,
        &wah_M->NonInlinedSFcns.inputOutputPortInfo2[0]);

      /* Set up the mdlInfo pointer */
      ssSetRTWSfcnInfo(rts, wah_M->sfcnInfo);

      /* Allocate memory of model methods 2 */
      {
        ssSetModelMethods2(rts, &wah_M->NonInlinedSFcns.methods2[0]);
      }

      /* Allocate memory of model methods 3 */
      {
        ssSetModelMethods3(rts, &wah_M->NonInlinedSFcns.methods3[0]);
      }

      /* Allocate memory of model methods 4 */
      {
        ssSetModelMethods4(rts, &wah_M->NonInlinedSFcns.methods4[0]);
      }

      /* Allocate memory for states auxilliary information */
      {
        ssSetStatesInfo2(rts, &wah_M->NonInlinedSFcns.statesInfo2[0]);
        ssSetPeriodicStatesInfo(rts, &wah_M->NonInlinedSFcns.periodicStatesInfo
          [0]);
      }
      ...
      /* outputs */
      {
        ssSetPortInfoForOutputs(rts,
          &wah_M->NonInlinedSFcns.Sfcn0.outputPortInfo[0]);
        ssSetPortInfoForOutputs(rts,
          &wah_M->NonInlinedSFcns.Sfcn0.outputPortInfo[0]);
        _ssSetNumOutputPorts(rts, 2);
        _ssSetPortInfo2ForOutputUnits(rts,
          &wah_M->NonInlinedSFcns.Sfcn0.outputPortUnits[0]);
        ssSetOutputPortUnit(rts, 0, 0);
        ssSetOutputPortUnit(rts, 1, 0);
        _ssSetPortInfo2ForOutputCoSimAttribute(rts,
          &wah_M->NonInlinedSFcns.Sfcn0.outputPortCoSimAttribute[0]);
        ...
        /* port 0 */
        {
          ...
          ssSetOutputPortSignal(rts, 0, ((int32_T *) wah_B.PiSoundInput_o1));
        }
        ...
        /* port 1 */
        {
          ...
          ssSetOutputPortSignal(rts, 1, ((int32_T *) wah_B.PiSoundInput_o2));
        }
      }

      /* path info */
      ssSetModelName(rts, "PiSound Input");
      ssSetPath(rts, "wah/PiSound Input");
      ssSetRTModel(rts,wah_M);
      ...
      /* parameters */
      {
        mxArray **sfcnParams = (mxArray **) &wah_M->NonInlinedSFcns.Sfcn0.params;
        ssSetSFcnParamsCount(rts, 3);
        ssSetSFcnParamsPtr(rts, &sfcnParams[0]);
        ssSetSFcnParam(rts, 0, (mxArray*)wah_P.PiSoundInput_P1_Size);
        ssSetSFcnParam(rts, 1, (mxArray*)wah_P.PiSoundInput_P2_Size);
        ssSetSFcnParam(rts, 2, (mxArray*)wah_P.PiSoundInput_P3_Size);
      }
    ...
    /* Level2 S-Function Block: wah/<Root>/PiSound Output (pisound_out) */
    {
      SimStruct *rts = wah_M->childSfunctions[1];

      /* timing info */
      time_T *sfcnPeriod = wah_M->NonInlinedSFcns.Sfcn1.sfcnPeriod;
      time_T *sfcnOffset = wah_M->NonInlinedSFcns.Sfcn1.sfcnOffset;
      int_T *sfcnTsMap = wah_M->NonInlinedSFcns.Sfcn1.sfcnTsMap;
      (void) memset((void*)sfcnPeriod, 0,
                    sizeof(time_T)*1);
      (void) memset((void*)sfcnOffset, 0,
                    sizeof(time_T)*1);
      ...
      {
        ssSetBlkInfo2Ptr(rts, &wah_M->NonInlinedSFcns.blkInfo2[1]);
      }

      _ssSetBlkInfo2PortInfo2Ptr(rts,
        &wah_M->NonInlinedSFcns.inputOutputPortInfo2[1]);
      ...
      /* Allocate memory of model methods 2 */
      {
        ssSetModelMethods2(rts, &wah_M->NonInlinedSFcns.methods2[1]);
      }

      /* Allocate memory of model methods 3 */
      {
        ssSetModelMethods3(rts, &wah_M->NonInlinedSFcns.methods3[1]);
      }

      /* Allocate memory of model methods 4 */
      {
        ssSetModelMethods4(rts, &wah_M->NonInlinedSFcns.methods4[1]);
      }

      /* Allocate memory for states auxilliary information */
      {
        ssSetStatesInfo2(rts, &wah_M->NonInlinedSFcns.statesInfo2[1]);
        ssSetPeriodicStatesInfo(rts, &wah_M->NonInlinedSFcns.periodicStatesInfo
          [1]);
      }
      ...
      /* inputs */
      {
        ...
        ssSetPortInfoForInputs(rts, &wah_M->NonInlinedSFcns.Sfcn1.inputPortInfo
          [0]);
        ssSetPortInfoForInputs(rts, &wah_M->NonInlinedSFcns.Sfcn1.inputPortInfo
          [0]);
        _ssSetPortInfo2ForInputUnits(rts,
          &wah_M->NonInlinedSFcns.Sfcn1.inputPortUnits[0]);
        ssSetInputPortUnit(rts, 0, 0);
        ssSetInputPortUnit(rts, 1, 0);
        _ssSetPortInfo2ForInputCoSimAttribute(rts,
          &wah_M->NonInlinedSFcns.Sfcn1.inputPortCoSimAttribute[0]);
        ...
        /* port 0 */
        {
          int32_T const **sfcnUPtrs = (int32_T const **)
            &wah_M->NonInlinedSFcns.Sfcn1.UPtrs0;

          {
            int_T i1;
            const int32_T *u0 = wah_B.DataTypeConversion2;
            for (i1=0; i1 < 128; i1++) {
              sfcnUPtrs[i1] = &u0[i1];
            }
          }
        ...
        {
          int32_T const **sfcnUPtrs = (int32_T const **)
            &wah_M->NonInlinedSFcns.Sfcn1.UPtrs1;

          {
            int_T i1;
            const int32_T *u1 = wah_B.DataTypeConversion3;
            for (i1=0; i1 < 128; i1++) {
              sfcnUPtrs[i1] = &u1[i1];
            }
          }
      ...
      ssSetPath(rts, "wah/PiSound Output");
      ssSetRTModel(rts,wah_M);
      ssSetParentSS(rts, (NULL));
      ssSetRootSS(rts, rts);
      ssSetVersion(rts, SIMSTRUCT_VERSION_LEVEL2);
      ...
      /* parameters */
      {
        mxArray **sfcnParams = (mxArray **) &wah_M->NonInlinedSFcns.Sfcn1.params;
        ssSetSFcnParamsCount(rts, 3);
        ssSetSFcnParamsPtr(rts, &sfcnParams[0]);
        ssSetSFcnParam(rts, 0, (mxArray*)wah_P.PiSoundOutput_P1_Size);
        ssSetSFcnParam(rts, 1, (mxArray*)wah_P.PiSoundOutput_P2_Size);
        ssSetSFcnParam(rts, 2, (mxArray*)wah_P.PiSoundOutput_P3_Size);
      }
...

  {
    ...
    /* Start for MATLABSystem: '<Root>/MATLAB System1' */
    /*  --- I/O */
    wah_DW.obj.Phase = 0.0;
    memset(&wah_DW.obj.WaveTables[0], 0, 40960U * sizeof(real_T));
    memset(&wah_DW.obj.TablesSize[0], 0, 10U * sizeof(real_T));
    for (b_k = 0; b_k < 1024; b_k++) {
      wah_B.sine[b_k] = sin(0.0061359231515425647 * (real_T)b_k);
    }

    for (b_k = 0; b_k < 5; b_k++) {
      wah_DW.obj.TablesSize[b_k] = 1024.0;
    }
    ...
    for (b_k = 0; b_k < 1024; b_k++) {
      wah_DW.obj.WaveTables[b_k] = wah_B.sine[b_k];
      wah_DW.obj.WaveTables[b_k + 4096] = tmp[b_k];
      wah_DW.obj.WaveTables[b_k + 8192] = tmp_0[b_k];
      wah_DW.obj.WaveTables[b_k + 12288] = 0.001953125 * (real_T)b_k - 1.0;
      wah_DW.obj.WaveTables[b_k + 16384] = -0.001953125 * (real_T)b_k + 1.0;
      wah_B.w4[b_k] = 0.0;
    }
    ...
    for (b_k = 0; b_k < 64; b_k++) {
      ...
      for (b_k_0 = 0; b_k_0 < 1024; b_k_0++) {
        wah_B.w4[b_k_0] += sin(0.0009765625 * (real_T)b_k_0 * a) * a_0;
      }
    }

    /*  5) “Organ” (impares), roll-off 1/k */
    memset(&wah_B.w5[0], 0, sizeof(real_T) << 10U);
    for (b_k_0 = 0; b_k_0 < 32; b_k_0++) {
      b_k = (b_k_0 << 1) + 1;
      a = 6.2831853071795862 * (real_T)b_k;
      a_0 = 1.0 / (real_T)b_k;
      for (b_k = 0; b_k < 1024; b_k++) {
        wah_B.w5[b_k] += sin(0.0009765625 * (real_T)b_k * a) * a_0;
      }
    }

    for (b_k = 0; b_k < 1024; b_k++) {
      wah_B.sine[b_k] = fabs(wah_B.w4[b_k]);
    }

    a = wah_B.sine[0];
    for (b_k = 0; b_k < 1023; b_k++) {
      a_0 = wah_B.sine[b_k + 1];
      if (a < a_0) {
        a = a_0;
      }
    }
    ...
    for (b_k = 0; b_k < 1024; b_k++) {
      ...
      wah_B.sine[b_k] = fabs(wah_B.w5[b_k]);
    }
    ...
    for (b_k = 0; b_k < 1024; b_k++) {
      ...
      wah_DW.obj.WaveTables[b_k + 20480] = tmp_1[b_k];
      wah_DW.obj.WaveTables[b_k + 24576] = tmp_2[b_k];
      wah_DW.obj.WaveTables[b_k + 28672] = tmp_3[b_k];
      wah_DW.obj.WaveTables[b_k + 32768] = wah_B.w4[b_k];
      wah_DW.obj.WaveTables[b_k + 36864] = wah_B.w5[b_k];
    }

    for (b_k = 0; b_k < 5; b_k++) {
      wah_DW.obj.TablesSize[b_k + 5] = 1024.0;
    }

    /* End of Start for MATLABSystem: '<Root>/MATLAB System1' */

    /* Start for MATLABSystem: '<Root>/MATLAB System' */
    wah_DW.obj_i.isInitialized = 1;
    ...
    wah_DW.obj_i.lastType = MAX_uint8_T;
    wah_DW.obj_i.lastFreq = (rtNaN);
    wah_DW.obj_i.lastQ = (rtNaN);
    wah_DW.obj_i.lastGain = (rtNaN);

    /* InitializeConditions for MATLABSystem: '<Root>/MATLAB System' */
    wah_DW.obj_i.zL[0] = 0.0;
    wah_DW.obj_i.zR[0] = 0.0;
    wah_DW.obj_i.zL[1] = 0.0;
    wah_DW.obj_i.zR[1] = 0.0;
  }
}

/* Model terminate function */
void wah_terminate(void)
{
