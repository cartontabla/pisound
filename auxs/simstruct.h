...
/* We have identifiers starting with '_' that can't be changed because of
 * compatibility */
...
#define ssSetNumInputPorts(S, nInputPorts)                                     \
  (*(S)->portInfo.regNumInputPortsFcn)((S)->portInfo.regNumInputPortsFcnArg,   \
                                       nInputPorts)
...
#define ssSetNumOutputPorts(S, nOutputPorts)                                   \
  (*(S)->portInfo.regNumOutputPortsFcn)((S)->portInfo.regNumOutputPortsFcnArg, \
                                        nOutputPorts)
#elif SS_SFCN_FOR_RTW
/* RTW S-function block */
#define ssSetNumOutputPorts(S, nOutputPorts)                                   \
 ...
 * @return the data type ID specified by id. Returns -1 if id is
 * DYNAMICALLY_TYPED
 */
 ...
 * @param S    SimStruct pointer
 * @param port input port index
 * @return COMPLEX_YES (1) if port accepts complex signals, COMPLEX_NO (0) if
 * port does not accept complex signals, and COMPLEX_INHERITED (-1) if port
 * inherits its numeric type from the port to which it is connected
 */
 ...
 * @param S    SimStruct pointer
 * @param port input port index
 * @param csig  Numeric type of the signals accepted by port. Valid values are
 * COMPLEX_NO (real signal), COMPLEX_YES (complex signal), and COMPLEX_INHERITED
 * (numeric type inherited from driving block).
 * @return 1 (COMPLEX_YES), 0 (COMPLEX_NO), or -1 (COMPLEX_INHERITED) depending
 ...
 * from thes sRegisterUnitFromExpr method. Returns DYNAMICALLY_TYPED if the
 * input port inherits its unit.
 */
 ...
 * @param    port          Index of the input port whose direct feedthrough
 * property is being set.
 * @param    dirFeed       Direct feedthrough status of the block specified by
 * port.
 */
#if !SS_DEBUG_FOR_SIM
 ...
 * @param    port          Index of the input port whose direct feedthrough
 * property is being set.
 * @return   The int_T value 1 if the input port specified by the index port has
 * direct feedthrough. Otherwise, returns 0.
 */
 ...
 * @return   A real_T value indicating the offset time of the input port
 ...
 * @param    S             SimStruct representing an S-Function block.
 * @param    expr          Function pointer corresponding to the unit expression
 * for the unit being registered.
 * @param    id            An integer whose value is the numeric unit type
 * identifier after the call to ssRegisterUnitFromExpr.
 */
#if !SS_SFCN_LEVEL_1
#if SS_SIM
 ...
 * @param    S             SimStruct representing an S-Function block.
 * @param    port          Index of the output port whose sample time is being
 * set.
 ...
 * @param    S             SimStruct representing an S-Function block.
 * @param    port          Index of the output port whose offset time is being
 * set.
 * @return   A real_T value indicating the offset time of the output port
 ...
 * @return   A value of type Frame_T, indicating if the input port specified by
 * the index port accepts signal frames.
 */
 ...
 * @param    S             SimStruct representing an S-Function block.
 * @param    port          Index of an output port.
 * @return   A value of type Frame_T, indicating if the output port specified by
 * the index port produces signal frames.
 */
...
#define ssGetOutputPortUnit(S, port)                                           \
  (S)->blkInfo.blkInfo2->portInfo2->outputUnits[(port)]                        \
      .unitId /*   (UnitId)    */
 ...
 * @param    S             SimStruct representing an S-Function block.
 * @param    port          Index of an output port.
 * @param   val           ID of the unit accepted by port. This ID is returned
 * from the ssRegisterUnitFromExpr method.
 * @return   The unit ID specified by id. Returns -1 if id is DYNAMICALLY_TYPED.
 */
...
#define ssAllowSignalsWithMoreThan2D(S)                                        \
  _ssSafelyCallGenericFcnStart(S)(S, GEN_FCN_SET_ALLOW_MORE_THAN_2D_SIGS, 1,   \
...
#define ssSetOneBasedIndexInputPort(S, pIdx)                                   \
  _ssSafelyCallGenericFcnStart(S)(S, GEN_FCN_SET_ONE_BASED_IN_PORT, pIdx,      \
                                  SS_NULL) _ssSafelyCallGenericFcnEnd
#endif
...
#define ssSetOneBasedIndexOutputPort(S, pIdx)                                  \
  _ssSafelyCallGenericFcnStart(S)(S, GEN_FCN_SET_ONE_BASED_OUT_PORT, pIdx,     \
                                  SS_NULL) _ssSafelyCallGenericFcnEnd
#endif
...
#define ssSetZeroBasedIndexInputPort(S, pIdx)                                  \
  _ssSafelyCallGenericFcnStart(S)(S, GEN_FCN_SET_ZERO_BASED_IN_PORT, pIdx,     \
                                  SS_NULL) _ssSafelyCallGenericFcnEnd
#endif
...
#define ssSetZeroBasedIndexOutputPort(S, pIdx)                                 \
  _ssSafelyCallGenericFcnStart(S)(S, GEN_FCN_SET_ZERO_BASED_OUT_PORT, pIdx,    \
                                  SS_NULL) _ssSafelyCallGenericFcnEnd
#endif
 ...
 *
 * Specify that an input port expects co-simulation signals that represents
 * continuous quantities
 * @param    S             SimStruct representing an S-Function block.
 * @param    pIdx          Input port of the S-function.
 * @param    val           Int or boolean value (0 or 1) that indicates if this
 * port expects a co-simulation signal, which is a discrete-time signal that
 * represents a continuous quantity.
 */
#define ssSetInputPortIsContinuousQuantity(S, port, val)                       \
  (S)->blkInfo.blkInfo2->portInfo2->inputCoSimAttribute[(port)]                \
      .isContinuousQuantity = (val)
#endif
 ...
 *
 * Determine whether an input port expects co-simulation signals that represents
 * continuous quantities
 * @param    S             SimStruct representing an S-Function block.
 * @param    pIdx          Input port of the S-function.
 * @return   Int or boolean value (0 or 1) that indicates if this port expects
 * a co-simulation signal, which is a discrete-time signal that represents a
 * continuous quantity.
 */
#define ssGetInputPortIsContinuousQuantity(S, port)                            \
  ((S)->blkInfo.blkInfo2->portInfo2->inputCoSimAttribute[(port)]               \
       .isContinuousQuantity == 1U)
#endif
 ...
 *
 * Specify that an output port generates co-simulation signals that represents
 * continuous quantities
 * @param    S             SimStruct representing an S-Function block.
 * @param    pIdx          Output port of the S-function.
 * @param    val           Int or boolean value (0 or 1) that indicates if this
 * port generates a co-simulation signal, which is a discrete-time signal that
 * represents a continuous quantity.
 */
#define ssSetOutputPortIsContinuousQuantity(S, port, val)                      \
  (S)->blkInfo.blkInfo2->portInfo2->outputCoSimAttribute[(port)]               \
      .isContinuousQuantity = (val)
#endif
 ...
 *
 * Determine whether an output port generates co-simulation signals that
 * represents continuous quantities
 * @param    S             SimStruct representing an S-Function block.
 * @param    pIdx          Output port of the S-function.
 ...
 * @param    S             SimStruct representing an S-Function block.
 * @param    port          Index of an input port.
 * @param    val           Structure of type DimsInfo_T that specifies the
 ...
 * @param    value         Enum value corresponding to the ports dimensions
 * mode.
 */
#if SS_SL_INTERNAL || SS_SFCN_FOR_SIM
#define ssSetInputPortDimensionsMode(S, pIdx, value)                           \
  {                                                                            \
    ...
    _ssSafelyCallGenericFcnStart(S)((S), GEN_FCN_SET_INPUT_DIMS_MODE, pIdx,    \
 ...
 *
 * Set the dimensions of output port outIdx to be equal to the dimensions of
 * input port inpIdx. This method is called from mdlSetWorkWidths
 * @param    S             SimStruct representing an S-Function block.
...
#define ssSetInputPortDimsSameAsOutputPortDims(S, inpIdx, outIdx)              \
  {                                                                            \
    int val = outIdx;                                                          \
    _ssSafelyCallGenericFcnStart(S)((S), GEN_FCN_SET_INPUT_DIMS_SAMEAS_OUTPUT, \
                                    (inpIdx), (&val))                          \
        _ssSafelyCallGenericFcnEnd;                                            \
  }
#else
 ...
 * @param    S             SimStruct representing an S-Function block.
 * @param    port          Index of an input port
 * @param    val1          Row dimension of matrix signals accepted by port or
 * DYNAMICALLY_SIZED.
 * @param    val2          Column dimension of matrix signals accepted by port
 ...
 * @return   An int_T value indicating the number of elements in the input
 * signal. If the number of elements is unknown, returns DYNAMICALLY_SIZED.
 */
 ...
 * @param    S             SimStruct representing an S-Function block.
 * @param    dimsInfo      Structure of type DimsInfo_T that specifies the
 * dimensionality of the signals accepted by port.
 */
#if SS_SL_INTERNAL || SS_SFCN_FOR_SIM
#define ssPruneNDMatrixSingletonDims(S, dimInfo)                               \
  _ssSafelyCallGenericFcnStart(S)(S, GEN_FCN_PRUN_TRAILING_DIMS, 0,            \
                                  (void *)dimInfo) _ssSafelyCallGenericFcnEnd
#else
#define ssPruneNDMatrixSingletonDims(S, dimInfo)
#endif
#endif
 ...
 *
 * Set the current size corresponding to dimension dIdx of the output signal at
 * port pIdx
 * @param    S             SimStruct representing an S-Function block.
...
#define ssSetCurrentOutputPortDimensions(S, pIdx, _dIdx, _val)                 \
  {                                                                            \
    ...
    dIdxVal.dIdx = _dIdx;                                                      \
    dIdxVal.dVal = _val;                                                       \
    _ssSafelyCallGenericFcnStart(S)((S), GEN_FCN_SET_CURR_OUTPUT_DIMS, pIdx,   \
                                    &dIdxVal) _ssSafelyCallGenericFcnEnd;      \
  }
...
#define ssSetOutputPortDimensionsMode(S, pIdx, value)                          \
  {                                                                            \
    DimensionsMode_T val = value;                                              \
    _ssSafelyCallGenericFcnStart(S)((S), GEN_FCN_SET_OUTPUT_DIMS_MODE, pIdx,   \
                                    &val) _ssSafelyCallGenericFcnEnd;          \
  }
#else
#define ssSetOutputPortDimensionsMode(S, pIdx, value)
 ...
 * @param    S             SimStruct representing an S-Function block.
 * @param    port          Index of an output port.
 * @param    val1          Row dimension of matrix signals emitted by port or
 * DYNAMICALLY_SIZED.
 * @param    val2          Column dimension of matrix signals emitted by port or
 * DYNAMICALLY_SIZED.
 * @return   1 if successful; otherwise, 0.
 */
#if SS_SFCN_FOR_SIM
 ...
 * @param    S             SimStruct representing an S-Function block.
 * @param    port          Index of an output port.
 * @return   An int_T value indicating the width of the output port specified by
 * the index port.
 */
 ...
 * @param    S             SimStruct representing an S-Function block.
 * @param    outIdx        Output port index
 * @param    ruleInfo      Structure containing the dimensions propagation
 * method information.
 */
#if SS_SFCN_FOR_SIM
#define ssAddOutputDimsDependencyRule(S, outIdx, ruleInfo)                     \
  {                                                                            \
    _ssSafelyCallGenericFcnStart(S)((S), GEN_FCN_ADD_DIMS_DEPEND_RULE,         \
                                    (outIdx), (ruleInfo))                      \
        _ssSafelyCallGenericFcnEnd;                                            \
  }
#else
...
#define ssAddVariableSizeSignalsRuntimeChecker(S, checker_type)                \
  {                                                                            \
    _ssSafelyCallGenericFcnStart(S)((S), GEN_FCN_ADD_VARDIMS_RUNTIME_CHECKER,  \
                                    (checker_type), SS_NULL)                   \
        _ssSafelyCallGenericFcnEnd;                                            \
  }
#endif
 ...
 * @param    S             SimStruct representing an S-Function block.
 * @param    fcn           Function pointer corresponding to the function being
 * registered.
 */
#if SS_SL_INTERNAL || SS_SFCN_FOR_SIM
#define ssRegMdlSetInputPortDimensionsModeFcn(S, fcn)                          \
  {                                                                            \
    ...
    _ssSafelyCallGenericFcnStart(S)((S), GEN_FCN_REG_SET_INPUT_DIMS_MODE_MTH,  \
                                    0, (void *)_mdlDimsModeFcn)                \
        _ssSafelyCallGenericFcnEnd;                                            \
  }
#else
#define ssRegMdlSetInputPortDimensionsModeFcn(S, fcn)
 ...
 * @param    n             Enum value corresponding to the signal size compute
 * type.
 */
 ...
 * @param    S             SimStruct representing an S-Function block.
 * @param    type          Enum value corresponding to the signal size compute
 * type.
 */
#if SS_SL_INTERNAL || SS_SFCN_FOR_SIM
#define ssSetSignalSizesComputeType(S, type)                                   \
  {                                                                            \
    _ssSafelyCallGenericFcnStart(S)((S),                                       \
                                    ...
                                    (type), 0) _ssSafelyCallGenericFcnEnd;     \
  }
#else
#define ssSetSignalSizesComputeType(S, type)
#endif
 ...
 *
 * Get the total width (total number of elements) of the signal at input port
 * pIdx
 * @param    S             SimStruct representing an S-Function block.
 * @param    portIdx          Input port index being polled.
 * @return   An int_T value indicating the current width of the signal at input
 * port portIdx.
 */
 ...
 * @param    S             SimStruct representing an S-Function block.
 * @param    port          Index of an input port
 * @return   A pointer (int_T *) to an array of integers. The array contains
 * elements with the value DYNAMICALLY_SIZED (-1) when the size of a dimension
 * is unknown.
 */
#if defined(USE_64BIT_FIELDS)
 ...
 * @return   An int_T value indicating the size of dimension, dIdx, at the input
 * port specified by port. Returns 1 if the dIdx is greater than or equal to the
 * number of input port dimensions.
 */
#if defined(USE_64BIT_FIELDS)
...
#define ssGetInputPortDimensionSize(S, port, dIdx)                             \
  ((dIdx) < ssGetInputPortNumDimensions((S), (port))                           \
       ? (S)->portInfo.inputs[(port)].dims[(dIdx)]                             \
       : 1)
#endif
 ...
 * @param    S             SimStruct representing an S-Function block.
 * @param    port          Index of an input port.
 * @return   A positive integer indicating the number of dimensions of the input
 * port specified by the index port, or DYNAMICALLY_SIZED, if the number of
 ...
 *
 * Get the current size of dimension dIdx of the signal at output port pIdx and
 * record this port index.
 * @param    S             SimStruct representing an S-Function block.
 * @param    pIdx          Output port index being polled.
 * @param    dIdx          Index of dimension being polled.
 * @return   An int_T value indicating the size of dimension dIdx
...
#define ssGetCurrentOutputPortDimensionsAndRecordIndex(S, pIdx, dIdx)          \
  *((_ssSafelyCallGenericFcnStart(S)(                                          \
        (S), GEN_FCN_ADD_OUTPUT_VARDIMS_PORT_INDEX, (pIdx), SS_NULL)           \
         _ssSafelyCallGenericFcnEnd)                                           \
 ...
 * @return   An int_T value indicating the current width of the signal at output
 * port portIdx.
 */
 ...
 * @param    port          Index of an output port.
 * @param    dIdx          Index of the dimension.
 * @return   An int_T value indicating the size of dimension, dIdx, at the
 * output port specified by port. Returns 1 if the dIdx is greater than or equal
 * to the number of output port dimensions.
 */
#if defined(USE_64BIT_FIELDS)
 ...
 * @param    S             SimStruct representing an S-Function block.
 * @param    port          Index of an output port
 * @return   A positive integer indicating the number of dimensions of the
 * output port specified by the index port, or DYNAMICALLY_SIZED, if the number
 * of dimensions is unknown.
 */
 ...
 * @return   The int_T index of the output port that reuses the memory buffer of
 * the input port indicated by the index inputPortIdx. If none of the
 * S-function's output ports reuse this input port buffer, returns
 * INVALID_PORT_IDX (-1).
 */
 ...
 * @return   Either an int_T (1 or 0) or boolean_T (true or false) value
 * indicating if the input port specified by the index port is connected to a
 * nonvirtual block.
 */
 ...
 *
 * Get the reusability setting of the memory allocated to the input port of an
 * S-function
 * @param    S             SimStruct representing an S-Function block.
 * @param    port          Index of an input port of S.
 * @return   One of the followings:
 * SS_NOT_REUSABLE_AND_GLOBAL,SS_REUSABLE_AND_LOCAL, SS_REUSABLE_AND_GLOBAL,
 ...
 *
 * Specify reusability of the memory allocated to the input port of an
 * S-function
 * @param    S             SimStruct representing an S-Function block.
 * @param    port          Index of an input port of S.
 * @param    val           One of the followings:
 * SS_NOT_REUSABLE_AND_GLOBAL,SS_REUSABLE_AND_LOCAL, SS_REUSABLE_AND_GLOBAL,
 * SS_NOT_REUSABLE_AND_LOCAL
 */
 ...
 * @param    S             SimStruct representing an S-Function block.
 * @param    port          Index of an input port whose overwritability is
 * required.
 * @return   An int_T (1 or 0) or boolean_T (true or false) value indicating if
 * the input port specified by the index port can be overwritten.
 */
 ...
 *
 * Specify whether one of an S-function's input ports can be overwritten by one
 * of its output ports
 * @param    S             SimStruct representing an S-Function block.
 * @param    port          Index of an input port whose overwritability is being
 * set.
 ...
 * @param    S             SimStruct representing an S-Function block.
 * @param    ip            Index of the port whose address is required.
 * @return   A pointer (real_T *) to a real signal on the input port specified
 * by the index ip.
 */
 ...
 * @return   Pointer to an array of signal element pointers for the specified
 * input port ip.
 */
 ...
 * @param    S             SimStruct representing an S-Function block.
 * @param    ip            Index of the input port
 * @return   Pointers to the elements of a signal of type double connected to
 * the input port specified by the index ip.
 */
 ...
 * @return   An int_T (1 or 0) or boolean_T (true or false) value indicating if
 * this output port signal is connected to a nonvirtual block.
 */
 ...
 * @return   An int_T (1 or 0) or boolean_T (true or false) value indicating if
 * this output port signal is being merged with other signals.
 */
 ...
 *
 * Get the reusability setting of the memory allocated to the output port of an
 * S-function
 * @param    S             SimStruct representing an S-Function block.
 * @param    port          Index of the output port
 ...
 *
 * Specify reusability of the memory allocated to the output port of an
 * S-function
 * @param    S             SimStruct representing an S-Function block.
 * @param    port          Index of the output port
 * @param    val           Permissible values:
 * SS_NOT_REUSABLE_AND_GLOBAL,SS_REUSABLE_AND_LOCAL, SS_REUSABLE_AND_GLOBAL,
 * SS_NOT_REUSABLE_AND_LOCAL
 */
 ...
 * @param    S             SimStruct representing an S-Function block.
 * @param    port          Index of the output port
 * @return   A contiguous real_T vector of length equal to the width of the
 * output port.
 */
 ...
 *
 * Specify whether an output port can share its memory buffer with an input
 * port.
 * @param    S             SimStruct representing an S-Function block.
...
#define ssSetOutputPortOverwritesInputPort(S, pIdx, val)                       \
  {                                                                            \
    int inpIdx = val;                                                          \
    _ssSafelyCallGenericFcnStart(S)(                                           \
        (S), GEN_FCN_SET_OUTPUT_OVERWRITE_INPUT_IDX, (pIdx), (&inpIdx))        \
        _ssSafelyCallGenericFcnEnd;                                            \
  }
#endif
 ...
 * @return   A time_T value indicating the sample time associated with the index
 * sti.
 */
 ...
 * @param    S             SimStruct representing an S-Function block.
 * @param    port          Index of the input port whose sample time is
 * required.
 * @return   The real_T value of the sample time of the input port specified by
 * the index port.
 */
 ...
 * @param    S             SimStruct representing an S-Function block.
 * @param    port          Index of the input port whose sample time is being
 * set.
 ...
 * @param    S             SimStruct representing an S-Function block.
 * @return   An int_T value indicating the total number of port-based and
 * block-based sample times. Returns -1 if the block has unspecified port-based
 * sample times.
 */
 ...
 * @return   The number of sample times specified by nSampleTimes, or -1 if
 ...
 * @param    S             SimStruct representing an S-Function block.
 * @param    port          Index of the input port whose sample time index is to
 * be returned.
 */
 ...
 * @param    S             SimStruct representing an S-Function block.
 * @param    port          Index of the output port
 * @return   A real_T value indicating the sample time of the output port
 ...
 * @param    S             SimStruct representing an S-Function block.
 * @param    port          Index of the output port
 * @return   An int_T value indicating the sample time index for the output port
 * specified by the index outputPortIdx. Returns CONSTANT_TID (-2) for constant
 * (inf) sample times.
 */
 ...
 * triggered subsystem
 * @param    S             SimStruct representing an S-Function block.
 * @return   The Boolean value true if S uses port-based sample times and
 * resides in a triggered subsystem. Otherwise, returns false.
 */
 ...
 * @param    S             SimStruct representing an S-Function block.
 * @param    dtype         Logical value indicating whether to allow a datatype
 * with deepcopy
 */
#if SS_SL_INTERNAL || SS_SFCN_FOR_SIM

#define ssSetDeepCopyCompliant(S, flag)                                        \
  _ssSafelyCallGenericFcnStart(S)(S, GEN_FCN_SET_DEEPCOPY_COMPLIANT, flag,     \
                                  SS_NULL) _ssSafelyCallGenericFcnEnd
#else
 ...
 *
 * Specify sample time behavior and tunability for S-function blocks with
 * port-based sample times
 * @param    S             SimStruct representing an S-Function block.
 * @param    flag          Logical value indicating whether to allow a sample
 * time of Inf for ports in the S-function.
 */
#if SS_SL_INTERNAL || SS_SFCN_FOR_SIM

#define ssSetParameterTuningCompliance(S, flag)                                \
  _ssSafelyCallGenericFcnStart(S)(S, GEN_FCN_SET_PRM_TUNING_COMPLIANCE, flag,  \
                                  SS_NULL) _ssSafelyCallGenericFcnEnd
#else
 ...
 *
 * Get the sample time behavior and tunability for S-function blocks with
 * port-based sample times
 * @param    S             SimStruct representing an S-Function block.
 */
#if SS_SFCN_FOR_SIM
 ...
 * @param    S             SimStruct representing an S-Function block.
 * @param    tid           Task ID
 * @return   The Boolean value true when the simulation is executing the
 ...
 * @param    tid           Task ID
 * @return   The Boolean value true when the simulation is executing in the task
 * represented by task ID tid. Otherwise, returns false.
 */
#if SS_MULTITASKING ||                                                         \
    (defined(RSIM_WITH_SOLVER_MULTITASKING) && RSIM_WITH_SOLVER_MULTITASKING)
 ...
 * @param    tid           Task ID
 * @return   The Boolean value true if a sample hit has occurred at sti1 and a
 * sample hit has also occurred at sti2 in the same time step. Otherwise,
 * returns false.
 */
...
#define ssIsSampleHitInTask(S, my_sti, tid)                                    \
  (ssGetPerTaskSampleHitsPtr(S))[ssGetSampleTimeTaskID(S, my_sti) +            \
                                 ((tid) * (ssGetNumRootSampleTimes(S)))]

#define ssIsSpecialSampleHit(S, my_sti, promoted_sti, tid)                     \
  (ssIsMajorTimeStep(S) &&                                                     \
   ssIsSampleHitInTask(S, my_sti, ssGetSampleTimeTaskID(S, promoted_sti)))
#else
#define ssIsSpecialSampleHit(S, my_sti, promoted_sti, tid)                     \
 ...
 * @return   The Boolean value true if both st and ot are equal to
 * INHERITED_SAMPLE_TIME. Otherwise, returns false.
 */
 ...
 *
 * Determine whether a sample time and offset value pair indicate a triggered or
 * asynchronous sample time
 * @param    st            The sample time.
 * @param    ot            The offset time.
 * @return   The Boolean value true if st is equal to INHERITED_SAMPLE_TIME (-1)
 * and ot is either INHERITED_SAMPLE_TIME (-1) or any other negative integer.
 */
 ...
 * @param    base       Sample time resolution of the block. This is the
 * smallest time step that the block can take Events of the block can be set to
 * occur at integral multiples of resolution.
 *
 */
#if SS_SL_INTERNAL || SS_SFCN_FOR_SIM
#define ssSetControllableSampleTime(S, sti, base)                              \
  {                                                                            \
    real_T tmp = base;                                                         \
    _ssSafelyCallGenericFcnStart(S)(S, GEN_FCN_SET_CONTROLLABLE_SAMPLE_TIME,   \
                                    (int)sti, &tmp)                            \
        _ssSafelyCallGenericFcnEnd;                                            \
  }
#else
 ...
 * @param    S             SimStruct representing an S-Function block.
 * @param    id            ID of the data type.
 * @return   An int_T value indicating the size of the data type specified by
 * id, if id is valid and the data type's size has been set. Otherwise, returns
 * INVALID_DTYPE_SIZE and reports an error.
 */
#if SS_SIM
 ...
 *
 * Get number of data types registered for this simulation, including build-in
 * types
 * @param    S             SimStruct representing an S-Function block.
 ...
 * @return   1 (true) if successful. Otherwise, returns 0 (false) and reports an
 * error.
 */
#if SS_SIM
 ...
 * @param    S             SimStruct representing an S-Function block.
 * @param    id            ID of the data type.
 * @return   A pointer (void *) to the zero representation of the data type
 * specified by id, if id is valid and the data type's size has been set.
 ...
 * @return   The ID of the custom data type specified by name if name is a
 * registered type name. Otherwise, returns INVALID_DTYPE_ID and reports an
 * error.
 */
#if SS_SIM
 ...
 * @return   The base data type ID associated with the data type alias specified
 * by id. Otherwise, returns SS_NULL and reports an error.
 */
#if SS_SIM
...
#else
#define ssGetDataTypeIdAliasedThruTo(S, id)                                    \
  ssGetDataTypeIdAliasedThruTo_cannot_be_used_in_RTW
#endif
 ...
 * @param    S             SimStruct representing an S-Function block.
 * @param    port          Index of an output port
 * @return   A pointer (void *) to the vector of signal elements output at the
 * port specified by the index port.
 */
...
#define ssGetSFcnParamDataType(S, index, result)                               \
  {                                                                            \
    _ssSafelyCallGenericFcnStart(S)(S, GEN_FCN_GET_PARAM_DATATYPE, (index),    \
...
#define ssRegisterStringDataType(S, maxStringLength)                           \
  (_ssSafelyCallGenericFcnStart(S)(S, GEN_FCN_REGISTER_STRING_TYPE,            \
                                   (int_T)(maxStringLength), SS_NULL)          \
       _ssSafelyCallGenericFcnEnd)
#else
#define ssRegisterStringDataType(S, maxStringLength)                           \
  ssRegisterStringDataType_cannot_be_used_in_RTW
#endif
 ...
 * @return   The Boolean value true if the data type is a string. Otherwise,
 * returns false.
 */
#if SS_SIM
#define ssIsStringDataType(S, ID)                                              \
  ((bool)_ssSafelyCallGenericFcnStart(S)(S, GEN_FCN_IS_STRING_TYPE, ID,        \
                                         SS_NULL) _ssSafelyCallGenericFcnEnd)
#else
 ...
 * @param    S         SimStruct representing an S-Function block.
 * @param    ID        ID of data type.
 * @return   A value of type size_T, indicating the maximum length of the
 * string. Or 0, if the string has no maximum length.
 */
#if SS_SIM
#define ssGetStringDataTypeMaxLength(S, ID)                                    \
  ((size_t)_ssSafelyCallGenericFcnStart(S)(                                    \
      S, GEN_FCN_GET_STRING_TYPE_MAX_LENGTH, ID, SS_NULL)                      \
       _ssSafelyCallGenericFcnEnd)
#else
#define ssGetStringDataTypeMaxLength(S, ID)                                    \
  ssGetStringDataTypeMaxLength_cannot_be_used_in_RTW
#endif
...
#define ssGetInputStringLength(S, portIdx)                                     \
  ((size_t)_ssSafelyCallGenericFcnStart(S)(S, GEN_FCN_GET_INPUT_STRING_LENGTH, \
                                           portIdx, SS_NULL)                   \
       _ssSafelyCallGenericFcnEnd)
#else
#define ssGetInputStringLength(S, ID)                                          \
  ssGetInputStringLength_cannot_be_used_in_RTW
#endif
...
#define ssWriteOutputString(S, portIdx, str)                                   \
  {                                                                            \
    const char_T *s = str;                                                     \
    _ssSafelyCallGenericFcnStart(S)(S, GEN_FCN_WRITE_STRING_OUTPUT, portIdx,   \
 ...
 * @return   The data type ID of an S-function parameter represented by a MATLAB
 * array. Returns INVALID_DTYPE_ID if the MATLAB data type does not map to any
 * built-in Simulink data type ID.
 */
#if SS_SFCN_FOR_SIM
#ifdef __cplusplus
extern "C" {
 ...
 * @param    S        SimStruct representing an S-Function block.
 * @return   An int_T value indicating the number of expected S-function
 * parameters
 */
 ...
 * @return   A pointer (const mxArray *) to the value of the S-function
 * parameter specified by index.
 */
#define ssGetSFcnParam(S, index)                                               \
  (SS_STATIC_CAST(const mxArray *,                                             \
                  _ssGetSFcnParam(S, index))) /* (const mxArray *)             \
                                               */
 ...
 * SS_PRM_NOT_TUNABLE (false / not tunable), or SS_PRM_SIM_ONLY_TUNABLE (tunable
...
#define ssSetSFcnParamUnit(S, index, val)                                      \
  {                                                                            \
    _ssSafelyCallGenericFcnStart(S)(S, GEN_FCN_SET_PARAM_UNIT, index,          \
                                    (void *)&val) _ssSafelyCallGenericFcnEnd;  \
  }
#else                                     /* RT or NRT */
...
#define ssGetSFcnParamUnit(S, index)                                           \
  {                                                                            \
      (S)->blkInfo.blkInfo2->sfcnParamInfo2->sfcnParamUnits                    \
          ? (S)->blkInfo.blkInfo2->sfcnParamInfo2                              \
                ->sfcnParamUnits[index] /* User Specified*/                    \
          : 0                           /* SL_UNSPECIFIED*/                    \
  }
#else                                /* RT or NRT */
 ...
 * @param    S         SimStruct representing an S-Function block.
 * @param    element   Index of the output port element corresponding to the
...
#define ssCallSystemWithTid(S, element, tid)                                   \
  (((S)->callSys.fcns[(element)] != SS_NULL)                                   \
       ? ((*(S)->callSys.fcns[(element)])((S)->callSys.args1[(element)],       \
                                          ...
                                          (tid))                               \
              ? ((ssGetErrorStatus(S) == SS_NULL) &&                           \
                 (ssGetLocalErrorStatus(S) == SS_NULL))                        \
              : (0))                                                           \
       : (1))
...
#define ssEnableSystemWithTid(S, element, tid)                                 \
  ((((S)->callSys.fcns[ssGetOutputPortWidth((S), 0) + (element)] != SS_NULL)   \
        ? ((*(S)->callSys.fcns[ssGetOutputPortWidth((S), 0) + (element)])(     \
               (S)->callSys.args1[(element)], (S)->callSys.args2[(element)],   \
               (tid))                                                          \
               ? ((ssGetErrorStatus(S) == SS_NULL) &&                          \
                  (ssGetLocalErrorStatus(S) == SS_NULL))                       \
               : (0))                                                          \
 ...
 * @param    S         SimStruct representing an S-Function block.
 * @param    elemIdx   The zero-based element index that is output on the first
 * output port
 * @return   int_T
 */
 ...
 *
 * Determine whether this S-function explicitly enables and disables the
 * function-call subsystems that it invokes
 * @param    S         SimStruct representing an S-Function block.
 * @return   The uint_T 1 if S explicitly enables or disables the
 * function-control subsystem that it invokes. Otherwise, returns 0.
 */
 ...
 *
 * Specify whether this S-function explicitly enables and disables the
 * function-call subsystem that it calls
 * @param    S         SimStruct representing an S-Function block.
 * @param    n         1 if this S-function explicitly enables and disables the
 * function-call subsystems it enables
 */
 ...
 * @param    S           SimStruct representing an S-Function block.
 * @return   The Boolean value true if the model reference Simulink Coder target
 * is building. Returns false if the model reference simulation target is
 * building.
 */
#if SS_SIM
 ...
 * @param    S           SimStruct representing an S-Function block.
 * @return   The Boolean value true if the model reference Simulation target is
 * building. Returns false if the model reference Simulink Coder target is
 * building.
 */
#if SS_SIM
 ...
 *
 * Specify if S-function can be used in referenced model simulating in normal
 * mode
 * @param    S    SimStruct representing an S-Function block.
 * @param    n    Flag for normal mode simulation support when the S-Function
 * block is used in a referenced model.
 */
 ...
 *
 * Specify that a referenced model containing this S-function can inherit its
 * sample time from its parent model
 * @param    S    SimStruct representing an S-Function block.
 */
 ...
 *
 * Specify that the use of this S-function in a referenced model prevents the
 * referenced model from inheriting its sample time from its parent model
 * @param    S    SimStruct representing an S-Function block.
 */
 ...
 *
 * Specify whether use of this S-function in a referenced model prevents the
 * referenced model from inheriting its sample time from its parent model
 * @param    S    SimStruct representing an S-Function block.
 * @param    n    Rule for allowing referenced models containing this S-function
 * to inherit their sample times from the parent model.
 */
 ...
 * @param    S           SimStruct representing an S-Function block.
 * @param    busTypeID   The bus data type identifier that represents the bus
 * signal
 * @param    elemIdx     The zero-based bus element index
 * @return   CSignal_T
 */
#define ssGetBusElementComplexSignal(S, busTypeID, elemIdx)                    \
  (dtaGetDataTypeElementSignalType(ssGetDataTypeAccess(S), ssGetPath(S),       \
                                   (busTypeID), (elemIdx)))
...
#define ssGetBusElementDataType(S, busTypeID, elemIdx)                         \
  (dtaGetDataTypeElementDataType(ssGetDataTypeAccess(S), ssGetPath(S),         \
                                 (busTypeID), (elemIdx)))
...
#define ssGetBusElementDimensions(S, busTypeID, elemIdx)                       \
  (dtaGetDataTypeElementDimensions(ssGetDataTypeAccess(S), ssGetPath(S),       \
                                   (busTypeID), (elemIdx)))
...
#define ssGetBusElementName(S, busTypeID, elemIdx)                             \
  (dtaGetDataTypeElementName(ssGetDataTypeAccess(S), ssGetPath(S),             \
                             (busTypeID), (elemIdx)))
...
#define ssGetBusElementNumDimensions(S, busTypeID, elemIdx)                    \
  (dtaGetDataTypeElementNumDimensions(ssGetDataTypeAccess(S), ssGetPath(S),    \
                                      (busTypeID), (elemIdx)))
...
#define ssGetBusElementOffset(S, busTypeID, elemIdx)                           \
  (dtaGetDataTypeElementOffset(ssGetDataTypeAccess(S), ssGetPath(S),           \
                               (busTypeID), (elemIdx)))
 ...
 * @param    result      Value of the parameter specified in the block dialog
 * box
 */
 ...
 *
 * Specify whether to convert the input bus signal for an S-function from
 * virtual to nonvirtual
 * @param    S           SimStruct representing an S-Function block.
 * @param    pIdx        The zero-based index of the input port of the
 * S-function block
 * @param    value       True, if the input signal is a virtual bus and should
 * be converted to a nonvirtual bus; false otherwise
 */
#define ssSetBusInputAsStruct(S, pIdx, value)                                  \
  {                                                                            \
    boolean_T tmp = value;                                                     \
    _ssSafelyCallGenericFcnStart(S)((S), GEN_FCN_SET_BUS_INPUT_AS_STRUCT,      \
                                    pIdx, &tmp) _ssSafelyCallGenericFcnEnd;    \
  }
/**
 * @brief ssSetBusOutputAsStruct
 *
 * Specify whether the output bus signal from an S-function must be virtual or
 * nonvirtual
 * @param    S           SimStruct representing an S-Function block.
 * @param    pIdx        The zero-based index of the output port of the
 * S-function block
 * @param    value       True, if the output signal is a nonvirtual bus and
 * should be converted to a virtual bus; false otherwise
 */
#define ssSetBusOutputAsStruct(S, pIdx, value)                                 \
  {                                                                            \
    boolean_T tmp = value;                                                     \
    _ssSafelyCallGenericFcnStart(S)((S), GEN_FCN_SET_BUS_OUTPUT_AS_STRUCT,     \
                                    pIdx, &tmp) _ssSafelyCallGenericFcnEnd;    \
  }
/**
 * @brief ssSetBusOutputObjectName
 *
 * Specify the name of the bus object that defines the structure and type of the
 * output bus signal
 * @param    S           SimStruct representing an S-Function block
 * @param    pIdx        The zero-based index of the output port of the
 * S-function block
...
#define ssSetBusOutputObjectName(S, pIdx, name)                                \
  _ssSafelyCallGenericFcnStart(S)((S), GEN_FCN_SET_BUS_OUTPUT_OBJECT_NAME,     \
                                  pIdx, name) _ssSafelyCallGenericFcnEnd
 ...
 * @param    S           SimStruct representing an S-Function block.
 * @param    element     Index of the element of the first output port that is
 * issuing a function call.
 */
 ...
 *
 * Register a data type that a parameter in the Simulink data type table
 * specifies
 * @param    S           SimStruct representing an S-Function block.
 * @param    idx         The zero-based index of the S-function parameter that
 * specifies the bus object name
 * @return   id          A pointer to an int_T that contains the Simulink ID of
 * the newly registered data type
 */
#define ssRegisterTypeFromParameter(S, idx, id)                                \
  {                                                                            \
    _ssSafelyCallGenericFcnStart(S)(S, GEN_FCN_REGISTER_TYPE_FROM_PARAMETER,   \
                                    (idx), (id)) _ssSafelyCallGenericFcnEnd;   \
  }
/**
 * @brief ssRegisterTypeFromParameter
 *
 * Register a custom data type from a Simulink.AliasType, Simulink.NumericType,
 * or Simulink.Bus object.
 * @param    S           SimStruct representing an S-Function block.
 * @param    n           Name of the Simulink object to assign to the custom
 * data type.
 * @return   id          An integer whose value is the numeric data type
 * identifier after the call to ssRegisterTypeFromNamedObject
 */
#define ssRegisterTypeFromNamedObject(S, n, id)                                \
  {                                                                            \
    ...
    _ssSafelyCallGenericFcnStart(S)(                                           \
        ...
        (void *)&_slRegisterTypeFromNameInfo) _ssSafelyCallGenericFcnEnd;      \
  }
 ...
 *
 * Set the absolute tolerance used by a variable-step solver for a specific
 * S-function continuous state.
 * @param    S           SimStruct representing an S-Function block.
 ...
 *
 * Get the absolute tolerance used by the variable-step solver for a specified
 * state
 * @param    S           SimStruct representing an S-Function block.
 * @param    idx         An index representing an S-function continuous state.
 */
 ...
 *
 * Determine whether a block has requrested block reduction before the
 ...
 * @param    S           SimStruct representing an S-Function block.
 * @param    n           If not zero, the Simulink engine should attempt to
 * reduce this block.
 */
 ...
 *
 * Specify the behavior of a Simulink S-function when saving and restoring the
 * operating point of a model containing the given S-function
 * @param    S           SimStruct representing an S-function block.
 * @param    set         Define how to treat an S-function simulation state when
 * saving and restoring the model operating point.
 */
 ...
 *
 * Specify whether to make the S-function's simulation state visible in the
 * operating point of the model.
 * @param    S           SimStruct representing an S-Function block.
 * @param    vis         Option to specify the visibility of the S-function
 * simulation state. The default is false; the simulation state is hidden.
 */
 ...
 * @param    S           SimStruct representing an S-Function block.
 */
#define ssGetGetOperatingPointFcn(S)                                           \
  (S)->states.modelMethods2->modelMethods4->mdlGetOperatingPoint
 ...
 * @param    S           SimStruct representing an S-Function block.
 */
#define ssGetSetOperatingPointFcn(S)                                           \
  (S)->states.modelMethods2->modelMethods4->mdlSetOperatingPoint
 ...
 * @param    S           SimStruct representing an S-Function block.
 * @param    val         Boolean value (boolean_T) telling Simulink to skip the
 * continuous state consistency check.
 */
 ...
 * @param    S           SimStruct representing an S-Function block.
 * @param    result      SS_SimStatus object that returns the current simulation
 * status.
 */
 ...
 * Returns false otherwise. Note: if ssGetSimMode returns "external", the result
 * is identical to that of ssIsExternalSim.
 */
#if SS_SIM
 ...
 * @param    S           SimStruct representing an S-Function block.
 * @return   The Boolean value true if the model has compiled or is compiling
 * for Accelerator mode simulation. Returns false otherwise.
 */
#if SS_SIM
 ...
 * @param    S           SimStruct representing an S-Function block.
 * @return   The Boolean value true if the model is being built or simulated in
 * the Rapid Accelerator mode. Returns false otherwise.
 */
 ...
 * @param    S           SimStruct representing an S-Function block or a
 * Simulink model.
 ...
 * @param    S           SimStruct representing an S-Function block or a
 * Simulink model
 * @param    val         Boolean value (int_T) specifying whether stopping the
 * simulation has been requested (1) or not (0).
 */
...
#define ssSetStopRequested(S, val)                                             \
  {                                                                            \
    ...
    _ssSafelyCallConstGenericFcnStart(S)((((const SimStruct *)(S))),           \
                                         GEN_FCN_SET_STOP_REQUESTED_SFUN, 0,   \
                                         &(val2)) _ssSafelyCallGenericFcnEnd;  \
 ...
 * @param    S           SimStruct representing an S-Function block or a
 * Simulink model
 * @return   The Boolean value true if the solver being used to solve S is a
 * variable-step solver. Otherwise, returns false.
 */
 ...
 *
 * Notify Simulink that the block's continuous outputs have become inconsistent
 * since the last call to mdlOutputs (even though time and inputs many have not
 * changed). This call will force Simulink to re-evaluate this block's
 * continuous outputs at the beginning of the next integration step.
 *
 * @param    S           SimStruct representing an S-Function block or a
 * Simulink model.
 */
 ...
 * @brief ssSetContTimeOutputInconsistentWithStateAtMajorStep
 *
 * Notify Simulink that the block's continuous states have changed at the
 * current major time step. This call will force Simulink to reset solver data
 * based on cached values of the continuous states at the beginning of the next
 * integration step.
 *
 * @param    S           SimStruct representing an S-Function block or a
 * Simulink model.
 ...
 * @param    S             SimStruct representing an S-Function block.
 * @return   The Boolean value true if the current simulation time is equal to
 * the simulation start time. Otherwise, returns false.
 */
#define ssIsFirstInitCond(S)                                                   \
  (!(((S)->root->mdlInfo->mdlFlags).firstInitCondCalled))
 ...
 * @param    S           SimStruct representing an S-Function block.
 * @return   The Boolean value true if the simulation is in a minor time step.
 ...
 * @param    S           SimStruct representing an S-Function block.
 * @return   The Boolean value true if the simulation is in a major time step.
 * Otherwise, returns false.
 */
 ...
 * @param    S           SimStruct representing an S-Function block.
 * @return   The Boolean value true if the simulation is in a mode update time
 * step. Otherwise, returns false.
 */

#define ssIsModeUpdateTimeStep(S)                                              \
  (((S)->mdlInfo->simTimeStep == MAJOR_TIME_STEP ||                            \
    ssIsMinorTimeStepWithModeChange(S)) &&                                     \
 ...
 * @param    S          SimStruct representing an S-Function block
 * @param    n          Boolean flag indicating whether the block must use
 * absolute time
 *
 */
...
#define ssGetElapseTime(S, dataPtr)                                            \
  _ssSafelyCallGenericFcnStart(S)(S, GEN_FCN_GET_ELAPSE_TIME, 0,               \
                                  (double *)dataPtr)                           \
      _ssSafelyCallGenericFcnEnd
...
#define ssGetElapseTimeCounter(S, dataPtr)                                     \
  _ssSafelyCallGenericFcnStart(S)(S, GEN_FCN_GET_ELAPSE_TIME_COUNTER, 0,       \
                                  (int *)dataPtr) _ssSafelyCallGenericFcnEnd
...
#define ssGetElapseTimeCounterDtype(S, dataPtr)                                \
  _ssSafelyCallGenericFcnStart(S)(S, GEN_FCN_GET_ELAPSE_TIME_COUNTER_DTYPE, 0, \
                                  (int *)dataPtr) _ssSafelyCallGenericFcnEnd
...
#define ssGetElapseTimeResolution(S, dataPtr)                                  \
  _ssSafelyCallGenericFcnStart(S)(S, GEN_FCN_GET_ELAPSE_TIME_RESOLUTION, 0,    \
                                  (double *)dataPtr)                           \
      _ssSafelyCallGenericFcnEnd
 ...
 * @param    S             SimStruct representing an S-Function block.
 * @param    timeSource    Source of time used by the asynchronous task created
 * by the block for the asynchronous task the S-function registers
...
#define ssSetTimeSource(S, timeSource)                                         \
  _ssSafelyCallGenericFcnStart(S)(S, GEN_FCN_SET_TIME_SOURCE, timeSource,      \
                                  SS_NULL) _ssSafelyCallGenericFcnEnd
 ...
 * @param    S           SimStruct representing an S-Function block.
 * @param    sti         Index of the sample time corresponding to the task for
 * which the current time is to be returned.
 * @return   A value of type time_T.
 */
#define ssGetTaskTime(S, sti)                                                  \
  (S)->mdlInfo->t[ssIsTIDInStInfo(S, sti) ? ssGetSampleTimeTaskID(S, sti)      \
                                          : 0] /*  (time_T)   */
 ...
 * @return A pointer to the ssParamRec describing the attributes of the run-time
 * parameter specified by the index param.
 */
...
#define ssRegAllTunableParamsAsRunTimeParams(S, nms)                           \
  _ssSafelyCallGenericFcnStart(S)(S, GEN_FCN_REG_ALL_TUNE_PRM_AS_RTP, 0,       \
                                  (void *)nms) _ssSafelyCallGenericFcnEnd
 ...
 * @dtIdArg  dtIdArg     Value of type DTypeId that specifies the data type of
...
#define ssSetNumRunTimeParams(S, num)                                          \
  _ssSafelyCallGenericFcnStart(S)(S, GEN_FCN_SET_NUM_RUN_TIME_PARAMS, num,     \
                                  SS_NULL) _ssSafelyCallGenericFcnEnd
#endif
#endif
 ...
 * @param    S           SimStruct representing an S-Function block.
 * @param    idx         Index of a run-time parameter
 * @param    p           ssParamRec structure containing the attributes of the
 * run-time parameter.
 */
#if SS_SFCN
#if SS_RTW
...
#define ssSetRunTimeParamInfo(S, idx, p)                                       \
  _ssSafelyCallGenericFcnStart(S)(S, GEN_FCN_REG_RUN_TIME_PARAM, idx,          \
                                  (void *)p) _ssSafelyCallGenericFcnEnd
#endif
#endif
...
#define ssUpdateAllTunableParamsAsRunTimeParams(S)                             \
  _ssSafelyCallGenericFcnStart(S)(S, GEN_FCN_UPDATE_ALL_TUNE_PRM_AS_RTP, 0,    \
                                  SS_NULL) _ssSafelyCallGenericFcnEnd
...
#define ssUpdateDlgParamAsRunTimeParam(S, rtIdxArg)                            \
  {                                                                            \
    _ssSafelyCallGenericFcnStart(S)(S, GEN_FCN_UPDATE_AND_CNV_RUN_TIME_PARAM,  \
                                    (rtIdxArg), SS_NULL)                       \
        _ssSafelyCallGenericFcnEnd;                                            \
  }
 ...
 */
#define ssUpdateRunTimeParamData(S, idx, d)                                    \
  ((S)->sfcnParams.rtp[idx])->data = (d)
...
#define ssUpdateRunTimeParamInfo(S, idx, p)                                    \
  _ssSafelyCallGenericFcnStart(S)(S, GEN_FCN_UPDATE_RUN_TIME_PARAM, idx,       \
                                  (void *)p) _ssSafelyCallGenericFcnEnd
 ...
 * @param    S             SimStruct representing an S-Function block.
 * @param    val           One of the following enumerated values that indicates
 * if the S-function is run-time thread-safe or not or unknown:
 ...
 * @return The Boolean value true if nDWork is zero or a positive integer;
 * otherwise, false.
 */
#if !SS_SFCN_LEVEL_1
#if SS_SFCN_FOR_SIM
...
#elif defined(USE_32BIT_FIELDS)
#define ssSetNumDWork(S, num)                                                  \
  (((S)->regDataType.setNumDWorkFcn != SS_NULL)                                \
       ? (*(S)->regDataType.setNumDWorkFcn)((S), num)                          \
       : (0))
#endif
#else
 ...
 * @param  S         SimStruct representing an S-Function block.
 * @return An int_T value indicating the number of DWork vectors in this
 * S-function.
 */
#if defined(USE_64BIT_FIELDS)
 ...
 * numbers
 * @param  S         SimStruct representing an S-Function block.
 * @param  index     Index of a data type work vector, where the index is one of
 * 0, 1, 2, ... ssGetNumDWork(S)-1.
 * @return COMPLEX_YES (1) if the specified vector contains complex numbers;
 ...
 * @param  index     Index of a data type work vector, where the index is one of
 * 0, 1, 2, ... ssGetNumDWork(S)-1.
 * @return The data type ID of the DWork vector specified by the index vector if
 * the data type is valid. Otherwise, returns INVALID_DTYPE_ID.
 */
 ...
 * @return The data type ID specified by dtID. Returns -1 if dtID is
 * DYNAMICALLY_TYPED.
 */
 ...
 * @param  index     Index of a data type work vector, where the index is one of
 * 0, 1, 2, ... ssGetNumDWork(S)-1.
 * @return A pointer (char_T *) to the character vector representing the name of
 * the DWork vector specified by the index vector.
 */
 ...
 * @param  index     Index of a data type work vector, where the index is one of
 * 0, 1, 2, ... ssGetNumDWork(S)-1.
 * @return One of the enumerated types defined for ssDWorkUsageType in
 * simstruc_types.h.
 */
 ...
 * @param  index     Index of a data type work vector, where the index is one of
 * 0, 1, 2, ... ssGetNumDWork(S)-1.
 * @return The int_T value 1 (SS_DWORK_USED_AS_DSTATE) if this vector is used to
 * store a block's discrete states. Otherwise, returns 0
 ...
 * @param  index     Index of a data type work vector, where the index is one of
 * 0, 1, 2, ... ssGetNumDWork(S)-1.
 * @param  val       How this vector is used. A value of 1 indicates that the
 * work vector is to be used to store the block's discrete states
 * (SS_DWORK_USED_AS_DSTATE), a value of 0 indicates that the work vector is to
 * be used as a work vector (SS_DWORK_USED_AS_DWORK).
 ...
 * @param  index     Index of a data type work vector, where the index is one of
 * 0, 1, 2, ... ssGetNumDWork(S)-1.
 * @return The int_T number of elements in the DWork vector specified by the
 * index vector.
 */
#if defined(USE_64BIT_FIELDS)
...
#define ssSetDWorkWidth(S, index, val)                                         \
  (S)->work.dWorkAux[(index)].widthSLSize = (val)
#elif defined(USE_32BIT_FIELDS)
 ...
 * @param  S         SimStruct representing an S-Function block or Simulink
 * model.
 * @return A pointer (real_T *) to an array containing the derivatives of the
 * continuous states of S, which can be a block or the model. Returns SS_NULL if
 ...
 * @param  S               SimStruct representing an S-Function block.
 * @return A pointer (real_T *) to the continuous state vector as an array of
 * length ssGetNumContStates(S). Returns SS_NULL if the S-function does not have
 * any continuous states.
 */
 ...
 * @param  S               SimStruct representing an S-Function block.
 * @return A pointer (real_T *) to the discrete state vector as an array of
 * length ssGetNumDiscStates(S). Returns SS_NULL if the S-function does not have
 * any discrete states.
 */
 ...
 * @param  S               SimStruct representing an S-Function block.
 * @return The discrete state vector as an array of real_T elements of length
...
#define ssGetNumContStates(S)                                                  \
  (S)->blkInfo.blkInfo2->blkInfoSLSize->sizes.numContStates
#elif defined(USE_32BIT_FIELDS)
 ...
 * @param  S            SimStruct representing an S-Function block.
 * @param  nContStates  Number of continuous states to be set for the block
 * represented by S.
 */
#if defined(USE_64BIT_FIELDS)
 ...
 * @param  S            SimStruct representing an S-Function block or a Simulink
 * model.
 * @return  An int_T value indicating the number of discrete states.
 */
#if defined(USE_64BIT_FIELDS)
#define ssGetNumDiscStates(S)                                                  \
  (S)->blkInfo.blkInfo2->blkInfoSLSize->sizes.numDiscStates
#elif defined(USE_32BIT_FIELDS)
 ...
 *
 * Specify the number of states for which a block detects zero crossings that
 * occur between sample points
 * @param  S               SimStruct representing an S-Function block.
 * @param  nNonsampledZCs  Number of nonsampled zero crossings that a block
 * detects.
 * @return The number of modes specified by nNonsampledZCs, or -1 if
 * DYNAMICALLY_SIZED.
 */
#if defined(USE_64BIT_FIELDS)
 ...
 * @return The number of elements specified by nRWork, or -1 if
 * DYNAMICALLY_SIZED.
 */
#if defined(USE_64BIT_FIELDS)
 ...
 * @return The real_T value stored in the RWork vector element specified by the
 * index idx.
 */
 ...
 * @return The int_T value stored in the idx element of the integer work vector
 * for this S-function. Returns SS_NULL if no value was assigned into the idx
 * element of the IWork vector.
 */
 ...
 * @param  S           SimStruct representing an S-Function block.
 * @param  nPWork      Number of elements to be allocated to the pointer work
 * vector of the block represented by S.
 * @return The number of elements specified by nPWork, or -1 if
 * DYNAMICALLY_SIZED.
 */
#if defined(USE_64BIT_FIELDS)
 ...
 * @param  S       SimStruct representing an S-Function block.
 * @param  n       Size of the mode vector for the block represented by S. Valid
...
#define ssSetNumModes(S, n)                                                    \
  (S)->blkInfo.blkInfo2->blkInfoSLSize->sizes.numModes = (n)
#elif defined(USE_32BIT_FIELDS)
 ...
 * @return An int_T value for the element of the mode vector specified by the
 * index idx. Returns SS_NULL if no value was assigned into the idx element of
 * the mode vector.
 */
 ...
 * ... ssGetNumDWork(S)-1.
 * @return A pointer (char_T *) to the character vector used as the Simulink
 * Coder identifier for the DWork vector specified by the index vector. Returns
 * SS_NULL if no Simulink Coder identifier is specified.
 */
 ...
 * @param  index   Index of the work vector, where the index is one of 0, 1, 2,
 * ... ssGetNumDWork(S)-1.
 * @param  n       Flag to control if the DWork vector resolves to a
 * Simulink.Signal object, either 0, 1, or 2.
 ...
 * @param  index   Index of the work vector, where the index is one of 0, 1, 2,
 * ... ssGetNumDWork(S)-1.
 * @param  val     Storage class of the work vector. Must be one of the values
 * enumerated by ssRTWStorageType.
 * @return The ssRTWStorageType value entered as sc. Invokes an error if sc is
 ...
 *
 * Get the C type qualifier used to declare a DWork vector in code generated
 * from the associated S-function
 * @param  S       SimStruct representing an S-Function block
 * @param  index   Index of the work vector, where the index is one of 0, 1, 2,
 * ... ssGetNumDWork(S)-1.
 * @return A pointer (char_T *) to a character vector indicating the C type
 * qualifier used to declare the DWork vector specified by the index vector.
 * Returns SS_NULL if no type qualifier is specified.
 */
 ...
 *
 * Specify the C type qualifier used to declare a DWork vector in code generated
 * from the associated S-function
 * @param  S       SimStruct representing an S-Function block
 * @param  index   Index of the work vector, where the index is one of 0, 1, 2,
 * ... ssGetNumDWork(S)-1.
 ...
 * @param  S       SimStruct representing an S-Function block. The block must be
 * either a source block (i.e., a block without input ports) or a sink block
 * (i.e., a block without output ports).
 * @return A pointer (char_T *) to the character vector indicating the name of
 * the S-function placement group.
 */
 ...
 * @param  S       SimStruct representing an S-Function block.
 * @return The Boolean value true if the model is generating code for any
 * purpose other than Accelerator mode simulation. Returns false if the model is
 * not generating code or is generating code for Accelerator mode simulation.
 */
#if SS_SIM
 ...
 * @return An int_T (1 or 0) or boolean_T (true or false) indicating the success
 ...
 */
#if defined(USE_32BIT_FIELDS)
extern int_T ssWriteRTWMx2dMatParam(SimStruct *S, const char_T *name,
                                    const void *rVal, const void *iVal,
                                    int_T dtInfo, int_T nRows, int_T nCols);
#else
extern int_T ssWriteRTWMx2dMatParam(SimStruct *S, const char_T *name,
                                    const void *rVal, const void *iVal,
                                    int_T dtInfo, SLSize nRows, SLSize nCols);
#endif
 ...
 */
#if defined(USE_32BIT_FIELDS)
extern int_T ssWriteRTWMxVectParam(SimStruct *S, const char_T *name,
                                   const void *rVal, const void *iVal,
                                   int_T dtInfo, int_T numEl);
#else
extern int_T ssWriteRTWMxVectParam(SimStruct *S, const char_T *name,
                                   const void *rVal, const void *iVal,
                                   int_T dtInfo, SLSize numEl);
#endif
/**
 ...
 * or failure of the function.
 */
extern int_T ssWriteRTWWorkVect(SimStruct *S, const char_T *vectName,
                                int_T nNames, ...);
 ...
 * @param msg Must be a ANSI C printf-style character vector with optional
 * variable replacement parameters.
 * @return A positive value indicating the number of bytes transmitted. Returns
 ...
 * @param  S      SimStruct representing an S-Function block.
 * @return A pointer (char_T *) to a character vector that identifies the last
 * error message.
 */
 ...
 *
 * @param  S     SimStruct pointer
 * @return The name of the S-function MEX-file associated with the block if S is
 * a SimStruct for an S-Function block. Otherwise return SS_NULL.
 */
#define ssGetSFunctionName(S)                                                  \
  (((S) == (S)->root) ? (const char_T *)SS_NULL                                \
 ...
 * @param  S     SimStruct representing an S-function block or a Simulink model.
 * @return A pointer (char_T *) to a character vector containing the path to a
 * block.
 */
 ...
 *
 * Specify that a For Each Subsystem block supports an S-function operating
 * inside the subsystem.
 * @param  S      SimStruct representing an S-Function block.
 * @param  value  A character vector have a value of either true or false. If
 * true, then the For Each Subsystem will support having an S-function operate
 * inside of it.
 */
...
#define ssSupportsMultipleExecInstances(S, value)                              \
  {                                                                            \
    if (ssGetOwnerBlock(S) != SS_NULL) {                                       \
      boolean_T val = value;                                                   \
      _ssSafelyCallGenericFcnStart(S)((S), GEN_FCN_SUP_MULTI_EXEC_INSTANCES,   \
                                      0, &val) _ssSafelyCallGenericFcnEnd;     \
    }                                                                          \
  }
#else
#define ssSupportsMultipleExecInstances(S, value)
...
#define ssQuerySimulinkFunction(S, scopedName, query)                          \
  ({                                                                           \
    ...
    _ssSafelyCallGenericFcnStart(S)((S), GEN_FCN_QUERY_SIMULINK_FUNCTION, 0,   \
                                    &locQueryInfo) _ssSafelyCallGenericFcnEnd; \
...
#define ssDeclareSimulinkFunction(S, proto, ptr, vis)                          \
  {                                                                            \
    ...
    _ssSafelyCallGenericFcnStart(S)((S), GEN_FCN_DECLARE_SIMULINK_FUNCTION, 0, \
                                    &locAttrInfo) _ssSafelyCallGenericFcnEnd;  \
  }
...
#define ssDeclareFunctionCaller(S, proto)                                      \
  {                                                                            \
    _ssSafelyCallGenericFcnStart(S)((S),                                       \
                                    ...
                                    0, proto) _ssSafelyCallGenericFcnEnd;      \
  }
 ...
 */
#define ssCallSimulinkFunction(S, scopedName, nArgin, argins, nArgout,         \
                               argouts)                                        \
  {                                                                            \
    ...
    _ssSafelyCallGenericFcnStart(S)((S), GEN_FCN_CALL_SIMULINK_FUNCTION, 0,    \
                                    &locExecArgs) _ssSafelyCallGenericFcnEnd;  \
  }
 ...
 *
 * @param  S          SimStruct representing an S-Function block
 * @param scopedName  Name of function (or scoped name when the function is
 * being called)
...
#define ssSetSimulinkFunctionArgComplexity(S, scopedName, arg, val)            \
  {                                                                            \
    ...
    _ssSafelyCallGenericFcnStart(S)((S), GEN_FCN_SIMULINK_FUNCTION_ARGUMENT,   \
                                    0, &locAttrInfo)                           \
        _ssSafelyCallGenericFcnEnd;                                            \
  }
 ...
 *
 * Get a list of selected output ports in the current graph.  These ports may be
 * used in ssCallSigListCreateFcn to access the data for these ports.
 *
 * @param S                SimStruct representing an S-Function block
 * @param voidBlock        S-Function block (see ssGetOwnerBlock)
 * @param sigSetOpt        SIGSET_GRAPH or SIGSET_GRAPH_N_CHILDREN
 * @param voidOutPortObjs  Array of port objects;  free using
 * ssCallGenericDestroyFcn
 * @param outnPortObjs     Number of ports returned
 */
#define ssCallSelectedSignalsFcn(S, voidBlock, sigSetOpt, voidOutPortObjs,     \
                                 outnPortObjs)                                 \
 ...
 *
 * Get the "signal list" for a given set of ports.  This is used to access data
 ...
 * @param voidOutSigList   SL_SigList returned; free using
 * ssCallSigListDestroyFcn
 */
#define ssCallSigListCreateFcn(S, voidBlock, nPorts, voidPortObjs,             \
