...
#include <math.h>
#include "tremolo3_private.h"
#include <string.h>

/* Block signals (default storage) */
B_tremolo3_T tremolo3_B;

/* Block states (default storage) */
DW_tremolo3_T tremolo3_DW;

/* Real-time model */
static RT_MODEL_tremolo3_T tremolo3_M_;
RT_MODEL_tremolo3_T *const tremolo3_M = &tremolo3_M_;

/* Forward declaration for local functions */
static real_T tremolo3_wt_read_lin(const real_T tbl[4096], real_T L, real_T ph);
static real_T tremolo3_wt_read_lin(const real_T tbl[4096], real_T L, real_T ph)
{
  real_T idx;
  real_T k;
...
void tremolo3_step(void)
{
  ...
  {
    SimStruct *rts = tremolo3_M->childSfunctions[0];
    sfcnOutputs(rts,0);
  }
  ...
  ws = tremolo3_P.WaveSel_rt;
  if (tremolo3_P.WaveSel_rt < 1) {
    ws = 1U;
  } else if (tremolo3_P.WaveSel_rt > 10) {
    ws = 10U;
  }

  L = tremolo3_DW.obj.TablesSize[ws - 1];
  if (L < 2.0) {
    L = 2.0;
  }

  inc = tremolo3_P.Rate_rt / 48000.0;
  ph = tremolo3_DW.obj.Phase;
  if (rtIsNaN(tremolo3_P.iPhase_rt) || rtIsInf(tremolo3_P.iPhase_rt)) {
    q = (rtNaN);
  } else {
    q = fabs(tremolo3_P.iPhase_rt / 6.2831853071795862);
    if (fabs(q - floor(q + 0.5)) > 2.2204460492503131E-16 * q) {
      q = fmod(tremolo3_P.iPhase_rt, 6.2831853071795862);
    } else {
      q = 0.0;
    }
  ...
  /*  cycles */
  m = (tremolo3_P.Constant12_Value != 0);
  for (b_k = 0; b_k < 128; b_k++) {
    vL = tremolo3_wt_read_lin(&tremolo3_DW.obj.WaveTables[(ws - 1) << 12], L, ph
      + q);
    if (m) {
      /*  unipolar 0..1 */
      vL = (vL + 1.0) * 0.5;
    ...
    rtb_MATLABSystem1_o1_0[b_k] = tremolo3_P.Constant8_Value * vL;
    ph += inc;
    if (ph >= 1.0) {
      ph--;
    }
   ...
   */
  L = tremolo3_P.Constant1_Value;
  if (tremolo3_P.Constant1_Value < 0.0) {
    L = 0.0;
  }
  ...
  for (b_k = 0; b_k < 128; b_k++) {
    if (tremolo3_P.Constant12_Value != 0) {
      /* MATLABSystem: '<Root>/MATLAB System1' */
      inc = rtb_MATLABSystem1_o1_0[b_k];
      if (inc < 0.0) {
        inc = 0.0;
      }
   ...
   */
  /*  g es Nx1 */
  if (tremolo3_P.Constant6_Value != 0) {
    for (b_k = 0; b_k < 128; b_k++) {
      rtb_g[b_k] = rt_powd_snf(10.0, rtb_g[b_k] / 20.0);
    }
  ...
  for (b_k = 0; b_k < 128; b_k++) {
     ...
     * */
    inc = floor(tremolo3_P.Gain_Gain * (real_T)tremolo3_B.PiSoundInput_o1[b_k] *
                L * tremolo3_P.Gain2_Gain);
    if (rtIsNaN(inc) || rtIsInf(inc)) {
      inc = 0.0;
    } else {
      inc = fmod(inc, 4.294967296E+9);
    }

    /* DataTypeConversion: '<Root>/Data Type Conversion2' */
    tremolo3_B.DataTypeConversion2[b_k] = inc < 0.0 ? -(int32_T)(uint32_T)-inc :
      (int32_T)(uint32_T)inc;
     ...
     *  MATLABSystem: '<Root>/MATLAB System2'
     * */
    inc = floor(tremolo3_P.Gain1_Gain * (real_T)tremolo3_B.PiSoundInput_o2[b_k] *
                L * tremolo3_P.Gain3_Gain);
    if (rtIsNaN(inc) || rtIsInf(inc)) {
      inc = 0.0;
    } else {
      inc = fmod(inc, 4.294967296E+9);
    ...
    tremolo3_B.DataTypeConversion3[b_k] = inc < 0.0 ? -(int32_T)(uint32_T)-inc :
      (int32_T)(uint32_T)inc;
  }
  ...
  {
    SimStruct *rts = tremolo3_M->childSfunctions[1];
    sfcnOutputs(rts,0);
  }
   ...
   */
  tremolo3_M->Timing.t[0] =
    ((time_T)(++tremolo3_M->Timing.clockTick0)) * tremolo3_M->Timing.stepSize0;
}

/* Model initialize function */
void tremolo3_initialize(void)
{
  /* Registration code */
  rtsiSetSolverName(&tremolo3_M->solverInfo,"FixedStepDiscrete");
  tremolo3_M->solverInfoPtr = (&tremolo3_M->solverInfo);

  /* Initialize timing info */
  {
    int_T *mdlTsMap = tremolo3_M->Timing.sampleTimeTaskIDArray;
    mdlTsMap[0] = 0;
    tremolo3_M->Timing.sampleTimeTaskIDPtr = (&mdlTsMap[0]);
    tremolo3_M->Timing.sampleTimes = (&tremolo3_M->Timing.sampleTimesArray[0]);
    tremolo3_M->Timing.offsetTimes = (&tremolo3_M->Timing.offsetTimesArray[0]);

    /* task periods */
    tremolo3_M->Timing.sampleTimes[0] = (0.0026666666666666666);

    /* task offsets */
    tremolo3_M->Timing.offsetTimes[0] = (0.0);
  }

  rtmSetTPtr(tremolo3_M, &tremolo3_M->Timing.tArray[0]);

  {
    int_T *mdlSampleHits = tremolo3_M->Timing.sampleHitArray;
    mdlSampleHits[0] = 1;
    tremolo3_M->Timing.sampleHits = (&mdlSampleHits[0]);
  }

  rtmSetTFinal(tremolo3_M, -1);
  tremolo3_M->Timing.stepSize0 = 0.0026666666666666666;
  tremolo3_M->solverInfoPtr = (&tremolo3_M->solverInfo);
  tremolo3_M->Timing.stepSize = (0.0026666666666666666);
  rtsiSetFixedStepSize(&tremolo3_M->solverInfo, 0.0026666666666666666);
  rtsiSetSolverMode(&tremolo3_M->solverInfo, SOLVER_MODE_SINGLETASKING);

  /* child S-Function registration */
  {
    RTWSfcnInfo *sfcnInfo = &tremolo3_M->NonInlinedSFcns.sfcnInfo;
    tremolo3_M->sfcnInfo = (sfcnInfo);
    rtssSetErrorStatusPtr(sfcnInfo, (&rtmGetErrorStatus(tremolo3_M)));
    tremolo3_M->Sizes.numSampTimes = (1);
    rtssSetNumRootSampTimesPtr(sfcnInfo, &tremolo3_M->Sizes.numSampTimes);
    tremolo3_M->NonInlinedSFcns.taskTimePtrs[0] = (&rtmGetTPtr(tremolo3_M)[0]);
    rtssSetTPtrPtr(sfcnInfo,tremolo3_M->NonInlinedSFcns.taskTimePtrs);
    rtssSetTStartPtr(sfcnInfo, &rtmGetTStart(tremolo3_M));
    rtssSetTFinalPtr(sfcnInfo, &rtmGetTFinal(tremolo3_M));
    rtssSetTimeOfLastOutputPtr(sfcnInfo, &rtmGetTimeOfLastOutput(tremolo3_M));
    rtssSetStepSizePtr(sfcnInfo, &tremolo3_M->Timing.stepSize);
    rtssSetStopRequestedPtr(sfcnInfo, &rtmGetStopRequested(tremolo3_M));
    rtssSetDerivCacheNeedsResetPtr(sfcnInfo, &tremolo3_M->derivCacheNeedsReset);
    rtssSetZCCacheNeedsResetPtr(sfcnInfo, &tremolo3_M->zCCacheNeedsReset);
    rtssSetContTimeOutputInconsistentWithStateAtMajorStepPtr(sfcnInfo,
      &tremolo3_M->CTOutputIncnstWithState);
    rtssSetSampleHitsPtr(sfcnInfo, &tremolo3_M->Timing.sampleHits);
    rtssSetPerTaskSampleHitsPtr(sfcnInfo, &tremolo3_M->Timing.perTaskSampleHits);
    rtssSetSimModePtr(sfcnInfo, &tremolo3_M->simMode);
    rtssSetSolverInfoPtr(sfcnInfo, &tremolo3_M->solverInfoPtr);
  }

  tremolo3_M->Sizes.numSFcns = (2);

  /* register each child */
  {
    (void) memset((void *)&tremolo3_M->NonInlinedSFcns.childSFunctions[0], 0,
                  2*sizeof(SimStruct));
    tremolo3_M->childSfunctions =
      (&tremolo3_M->NonInlinedSFcns.childSFunctionPtrs[0]);
    tremolo3_M->childSfunctions[0] =
      (&tremolo3_M->NonInlinedSFcns.childSFunctions[0]);
    tremolo3_M->childSfunctions[1] =
      (&tremolo3_M->NonInlinedSFcns.childSFunctions[1]);

    /* Level2 S-Function Block: tremolo3/<Root>/PiSound Input (pisound_in) */
    {
      SimStruct *rts = tremolo3_M->childSfunctions[0];

      /* timing info */
      time_T *sfcnPeriod = tremolo3_M->NonInlinedSFcns.Sfcn0.sfcnPeriod;
      time_T *sfcnOffset = tremolo3_M->NonInlinedSFcns.Sfcn0.sfcnOffset;
      int_T *sfcnTsMap = tremolo3_M->NonInlinedSFcns.Sfcn0.sfcnTsMap;
      ...
      {
        ssSetBlkInfo2Ptr(rts, &tremolo3_M->NonInlinedSFcns.blkInfo2[0]);
      }

      _ssSetBlkInfo2PortInfo2Ptr(rts,
        &tremolo3_M->NonInlinedSFcns.inputOutputPortInfo2[0]);

      /* Set up the mdlInfo pointer */
      ssSetRTWSfcnInfo(rts, tremolo3_M->sfcnInfo);

      /* Allocate memory of model methods 2 */
      {
        ssSetModelMethods2(rts, &tremolo3_M->NonInlinedSFcns.methods2[0]);
      }

      /* Allocate memory of model methods 3 */
      {
        ssSetModelMethods3(rts, &tremolo3_M->NonInlinedSFcns.methods3[0]);
      }

      /* Allocate memory of model methods 4 */
      {
        ssSetModelMethods4(rts, &tremolo3_M->NonInlinedSFcns.methods4[0]);
      }

      /* Allocate memory for states auxilliary information */
      {
        ssSetStatesInfo2(rts, &tremolo3_M->NonInlinedSFcns.statesInfo2[0]);
        ssSetPeriodicStatesInfo(rts,
          &tremolo3_M->NonInlinedSFcns.periodicStatesInfo[0]);
      }

      /* outputs */
      {
        ssSetPortInfoForOutputs(rts,
          &tremolo3_M->NonInlinedSFcns.Sfcn0.outputPortInfo[0]);
        ssSetPortInfoForOutputs(rts,
          &tremolo3_M->NonInlinedSFcns.Sfcn0.outputPortInfo[0]);
        _ssSetNumOutputPorts(rts, 2);
        _ssSetPortInfo2ForOutputUnits(rts,
          &tremolo3_M->NonInlinedSFcns.Sfcn0.outputPortUnits[0]);
        ssSetOutputPortUnit(rts, 0, 0);
        ssSetOutputPortUnit(rts, 1, 0);
        _ssSetPortInfo2ForOutputCoSimAttribute(rts,
          &tremolo3_M->NonInlinedSFcns.Sfcn0.outputPortCoSimAttribute[0]);
        ...
        /* port 0 */
        {
          ...
          ssSetOutputPortSignal(rts, 0, ((int32_T *) tremolo3_B.PiSoundInput_o1));
        }
        ...
        /* port 1 */
        {
          ...
          ssSetOutputPortSignal(rts, 1, ((int32_T *) tremolo3_B.PiSoundInput_o2));
        }
      }

      /* path info */
      ssSetModelName(rts, "PiSound Input");
      ssSetPath(rts, "tremolo3/PiSound Input");
      ssSetRTModel(rts,tremolo3_M);
      ...
      /* parameters */
      {
        mxArray **sfcnParams = (mxArray **)
          &tremolo3_M->NonInlinedSFcns.Sfcn0.params;
        ssSetSFcnParamsCount(rts, 3);
        ssSetSFcnParamsPtr(rts, &sfcnParams[0]);
        ssSetSFcnParam(rts, 0, (mxArray*)tremolo3_P.PiSoundInput_P1_Size);
        ssSetSFcnParam(rts, 1, (mxArray*)tremolo3_P.PiSoundInput_P2_Size);
        ssSetSFcnParam(rts, 2, (mxArray*)tremolo3_P.PiSoundInput_P3_Size);
      }
    ...
    /* Level2 S-Function Block: tremolo3/<Root>/PiSound Output (pisound_out) */
    {
      SimStruct *rts = tremolo3_M->childSfunctions[1];

      /* timing info */
      time_T *sfcnPeriod = tremolo3_M->NonInlinedSFcns.Sfcn1.sfcnPeriod;
      time_T *sfcnOffset = tremolo3_M->NonInlinedSFcns.Sfcn1.sfcnOffset;
      int_T *sfcnTsMap = tremolo3_M->NonInlinedSFcns.Sfcn1.sfcnTsMap;
      (void) memset((void*)sfcnPeriod, 0,
                    sizeof(time_T)*1);
      (void) memset((void*)sfcnOffset, 0,
                    sizeof(time_T)*1);
      ...
      {
        ssSetBlkInfo2Ptr(rts, &tremolo3_M->NonInlinedSFcns.blkInfo2[1]);
      }

      _ssSetBlkInfo2PortInfo2Ptr(rts,
        &tremolo3_M->NonInlinedSFcns.inputOutputPortInfo2[1]);
      ...
      /* Allocate memory of model methods 2 */
      {
        ssSetModelMethods2(rts, &tremolo3_M->NonInlinedSFcns.methods2[1]);
      }

      /* Allocate memory of model methods 3 */
      {
        ssSetModelMethods3(rts, &tremolo3_M->NonInlinedSFcns.methods3[1]);
      }

      /* Allocate memory of model methods 4 */
      {
        ssSetModelMethods4(rts, &tremolo3_M->NonInlinedSFcns.methods4[1]);
      }

      /* Allocate memory for states auxilliary information */
      {
        ssSetStatesInfo2(rts, &tremolo3_M->NonInlinedSFcns.statesInfo2[1]);
        ssSetPeriodicStatesInfo(rts,
          &tremolo3_M->NonInlinedSFcns.periodicStatesInfo[1]);
      }
      ...
      /* inputs */
      {
        ...
        ssSetPortInfoForInputs(rts,
          &tremolo3_M->NonInlinedSFcns.Sfcn1.inputPortInfo[0]);
        ssSetPortInfoForInputs(rts,
          &tremolo3_M->NonInlinedSFcns.Sfcn1.inputPortInfo[0]);
        _ssSetPortInfo2ForInputUnits(rts,
          &tremolo3_M->NonInlinedSFcns.Sfcn1.inputPortUnits[0]);
        ssSetInputPortUnit(rts, 0, 0);
        ssSetInputPortUnit(rts, 1, 0);
        _ssSetPortInfo2ForInputCoSimAttribute(rts,
          &tremolo3_M->NonInlinedSFcns.Sfcn1.inputPortCoSimAttribute[0]);
        ...
        /* port 0 */
        {
          int32_T const **sfcnUPtrs = (int32_T const **)
            &tremolo3_M->NonInlinedSFcns.Sfcn1.UPtrs0;

          {
            int_T i1;
            const int32_T *u0 = tremolo3_B.DataTypeConversion2;
            for (i1=0; i1 < 128; i1++) {
              sfcnUPtrs[i1] = &u0[i1];
            }
          }
        ...
        {
          int32_T const **sfcnUPtrs = (int32_T const **)
            &tremolo3_M->NonInlinedSFcns.Sfcn1.UPtrs1;

          {
            int_T i1;
            const int32_T *u1 = tremolo3_B.DataTypeConversion3;
            for (i1=0; i1 < 128; i1++) {
              sfcnUPtrs[i1] = &u1[i1];
            }
          }
      ...
      ssSetPath(rts, "tremolo3/PiSound Output");
      ssSetRTModel(rts,tremolo3_M);
      ssSetParentSS(rts, (NULL));
      ssSetRootSS(rts, rts);
      ssSetVersion(rts, SIMSTRUCT_VERSION_LEVEL2);
      ...
      {
        mxArray **sfcnParams = (mxArray **)
          &tremolo3_M->NonInlinedSFcns.Sfcn1.params;
        ssSetSFcnParamsCount(rts, 3);
        ssSetSFcnParamsPtr(rts, &sfcnParams[0]);
        ssSetSFcnParam(rts, 0, (mxArray*)tremolo3_P.PiSoundOutput_P1_Size);
        ssSetSFcnParam(rts, 1, (mxArray*)tremolo3_P.PiSoundOutput_P2_Size);
        ssSetSFcnParam(rts, 2, (mxArray*)tremolo3_P.PiSoundOutput_P3_Size);
      }
...

  {
    ...
    /* Start for MATLABSystem: '<Root>/MATLAB System1' */
    /*  --- I/O */
    tremolo3_DW.obj.Phase = 0.0;
    memset(&tremolo3_DW.obj.WaveTables[0], 0, 40960U * sizeof(real_T));
    memset(&tremolo3_DW.obj.TablesSize[0], 0, 10U * sizeof(real_T));
    for (b_k = 0; b_k < 1024; b_k++) {
      tremolo3_B.sine[b_k] = sin(0.0061359231515425647 * (real_T)b_k);
    }

    for (b_k = 0; b_k < 5; b_k++) {
      tremolo3_DW.obj.TablesSize[b_k] = 1024.0;
    }
    ...
    for (b_k = 0; b_k < 1024; b_k++) {
      tremolo3_DW.obj.WaveTables[b_k] = tremolo3_B.sine[b_k];
      tremolo3_DW.obj.WaveTables[b_k + 4096] = tmp[b_k];
      tremolo3_DW.obj.WaveTables[b_k + 8192] = tmp_0[b_k];
      tremolo3_DW.obj.WaveTables[b_k + 12288] = 0.001953125 * (real_T)b_k - 1.0;
      tremolo3_DW.obj.WaveTables[b_k + 16384] = -0.001953125 * (real_T)b_k + 1.0;
      tremolo3_B.w4[b_k] = 0.0;
    }
    ...
    for (b_k = 0; b_k < 64; b_k++) {
      ...
      for (b_k_0 = 0; b_k_0 < 1024; b_k_0++) {
        tremolo3_B.w4[b_k_0] += sin(0.0009765625 * (real_T)b_k_0 * a) * a_0;
      }
    }

    /*  5) “Organ” (impares), roll-off 1/k */
    memset(&tremolo3_B.w5[0], 0, sizeof(real_T) << 10U);
    for (b_k_0 = 0; b_k_0 < 32; b_k_0++) {
      b_k = (b_k_0 << 1) + 1;
      a = 6.2831853071795862 * (real_T)b_k;
      a_0 = 1.0 / (real_T)b_k;
      for (b_k = 0; b_k < 1024; b_k++) {
        tremolo3_B.w5[b_k] += sin(0.0009765625 * (real_T)b_k * a) * a_0;
      }
    }

    for (b_k = 0; b_k < 1024; b_k++) {
      tremolo3_B.sine[b_k] = fabs(tremolo3_B.w4[b_k]);
    }

    a = tremolo3_B.sine[0];
    for (b_k = 0; b_k < 1023; b_k++) {
      a_0 = tremolo3_B.sine[b_k + 1];
      if (a < a_0) {
        a = a_0;
      }
    }
    ...
    for (b_k = 0; b_k < 1024; b_k++) {
      ...
      tremolo3_B.sine[b_k] = fabs(tremolo3_B.w5[b_k]);
    }
    ...
    for (b_k = 0; b_k < 1024; b_k++) {
      ...
      tremolo3_DW.obj.WaveTables[b_k + 20480] = tmp_1[b_k];
      tremolo3_DW.obj.WaveTables[b_k + 24576] = tmp_2[b_k];
      tremolo3_DW.obj.WaveTables[b_k + 28672] = tmp_3[b_k];
      tremolo3_DW.obj.WaveTables[b_k + 32768] = tremolo3_B.w4[b_k];
      tremolo3_DW.obj.WaveTables[b_k + 36864] = tremolo3_B.w5[b_k];
    }

    for (b_k = 0; b_k < 5; b_k++) {
      tremolo3_DW.obj.TablesSize[b_k + 5] = 1024.0;
    }
